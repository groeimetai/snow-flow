...
Most popular providers are preloaded by default. If you've added the credentials for a provider through `snow-flow auth login`, they'll be available when you start Snow-Flow.
...
Here are several models, in no particular order, that work well with Snow-Flow (to name a few):
...
```jsonc title=".mcp.jsonc" {7-12,19-24}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "openai": {
