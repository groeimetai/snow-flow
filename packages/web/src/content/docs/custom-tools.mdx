---
title: Custom Tools
description: Create tools the LLM can call in snow-code.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside snow-code's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.snow-code/tools/` directory of your project.
- Or globally, by placing them in `~/.config/snow-code/tools/`.
...
```ts title=".snow-code/tools/database.ts" {1}
import { tool } from "@snow-code-ai/plugin"
...
```ts title=".snow-code/tools/math.ts"
import { tool } from "@snow-code-ai/plugin"
...
```ts title=".snow-code/tools/project.ts" {8}
import { tool } from "@snow-code-ai/plugin"
...
```python title=".snow-code/tools/add.py"
import sys
...
```ts title=".snow-code/tools/python-add.ts" {10}
import { tool } from "@snow-code-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".snow-code/tools/add.py")
