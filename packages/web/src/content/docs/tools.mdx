...
Tools allow the LLM to perform actions in your codebase. Snow-Flow comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Snow-Flow.
...
:::note
This tool is only available when `SNOW_CODE_EXPERIMENTAL_LSP_TOOL=true` (or `SNOW_CODE_EXPERIMENTAL=true`).
:::
