...
With Snow-Flow you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Snow-Flow uses our own `snow-code` theme.
...
Snow-Flow comes with several built-in themes.
...
- Want Snow-Flow to match their terminal's appearance
...
Snow-Flow supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/snow-code/themes/*.json` or `$XDG_CONFIG_HOME/snow-code/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.snow-code/themes/*.json`
4. **Current working directory** - Defined in `./.snow-code/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/snow-code/themes
vim ~/.config/snow-code/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .snow-code/themes
vim .snow-code/themes/my-theme.json
```
...
{
  "$schema": "https://snow-flow.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
