...
By default, Snow-Flow uses our own `snow-flow` theme.
...
2. **User config directory** - Defined in `~/.config/snow-flow/themes/*.json` or `$XDG_CONFIG_HOME/snow-flow/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.snow-flow/themes/*.json`
4. **Current working directory** - Defined in `./.snow-flow/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/snow-flow/themes
vim ~/.config/snow-flow/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .snow-flow/themes
vim .snow-flow/themes/my-theme.json
```
