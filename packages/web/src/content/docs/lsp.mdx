...
:::note
You can disable automatic LSP server downloads by setting the `SNOW_FLOW_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When snow-flow opens a file, it:
...
You can customize LSP servers through the `lsp` section in your snow-flow config.
