---
title: LSP Servers
description: Snow-Flow integrates with your LSP servers.
---

Snow-Flow integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

Snow-Flow comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `SNOW_CODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When snow-code opens a file, it:
...
You can customize LSP servers through the `lsp` section in your snow-code config.
