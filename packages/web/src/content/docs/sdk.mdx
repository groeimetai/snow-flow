---
title: SDK
description: Type-safe JS client for snow-flow server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The snow-flow JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control snow-flow programmatically.
...
```bash
npm install @groeimetai/snow-flow-sdk
```
...
Create an instance of snow-flow:

```javascript
import { createOpencode } from "@groeimetai/snow-flow-sdk"
...
You can pass a configuration object to customize behavior. The instance still picks up your `.mcp.json`, but you can override or add configuration inline:

```javascript
import { createOpencode } from "@groeimetai/snow-flow-sdk"

const snow-flow = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${snow-flow.server.url}`)

snow-flow.server.close()
```

## Client only

If you already have a running instance of snow-flow, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@groeimetai/snow-flow-sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*snow-flow" },
})
