---
title: SDK
description: Type-safe JS client for snow-code server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The snow-code JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control snow-code programmatically.
...
Create an instance of snow-code:

```javascript
import { createOpencode } from "@snow-code-ai/sdk"
...
You can pass a configuration object to customize behavior. The instance still picks up your `snowcode.json`, but you can override or add configuration inline:

```javascript
import { createOpencode } from "@snow-code-ai/sdk"

const snow-code = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${snow-code.server.url}`)

snow-code.server.close()
```

## Client only

If you already have a running instance of snow-code, you can create a client instance to connect to it:

```javascript
import { createOpencodeClient } from "@snow-code-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@snow-code-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*snow-code" },
})
