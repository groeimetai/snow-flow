...
2. You run `snow-flow auth login`, select snow-flow, and paste your API key.
...
| Grok Code Fast 1  | grok-code         | `https://snow-flow.dev/zen/v1/chat/completions` | `@ai-sdk/openai-compatible` |
...
uses the format `snow-flow/<model-id>`. For example, for GPT 5 Codex, you would
use `snow-flow/gpt-5-codex` in your config.
...
- Grok Code Fast 1 is currently free on Snow-Flow for a limited time. The xAI team is using this time to collect feedback and improve Grok Code.
- Code Supernova is a stealth model that's free on Snow-Flow for a limited time. The team is using this time to collect feedback and improve the model.
