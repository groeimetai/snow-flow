---
title: TUI
description: Using the Snow-Flow terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Snow-Flow provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Snow-Flow starts the TUI for the current directory.
...
When using the Snow-Flow TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
List available agents and switch between them.
...
Authenticate with LLM providers and ServiceNow. _Alias_: `/login`
...
Exit Snow-Flow. _Aliases_: `/quit`, `/q`
...
Generate a session report with token usage, costs, and conversation summary.
...
Toggle display of thinking blocks (for models that support extended thinking).
...
Show session timeline and history. _Alias_: `/history`
...
You can customize TUI behavior through your Snow-Flow config file.
