...
There are two types of agents in Snow-Flow; primary agents and subagents.
...
Snow-Flow comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Snow-Flow comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Snow-Flow comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `snowcode.json` config file:
...
- Global: `~/.config/snow-code/agents/`
- Per-project: `.snow-code/agents/`

```markdown title="~/.config/snow-code/agents/review.md"
---
...
If no temperature is specified, Snow-Flow uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Snow-Flow config and the project specific config.
...
The model ID in your Snow-Flow config uses the format `provider/model-id`. For example, if you're using [Snow-Flow Zen](/docs/zen), you would use `snow-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="snowcode.json" {3-6,9-12}
{
  "$schema": "https://snow-flow.dev/config.json",
  "tools": {
    "write": true,
...
```json title="snowcode.json" {3-5,8-10}
{
  "$schema": "https://snow-flow.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `snow-code models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/groeimetai/snow-flow).
:::
...
```markdown title="~/.config/snow-code/agents/docs-writer.md"
---
...
```markdown title="~/.config/snow-code/agents/security-auditor.md"
---
