...
To debug any issues with Snow-Flow, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/snow-flow/log/`
- **Windows**: `%USERPROFILE%\.local\share\snow-flow\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `snow-flow --log-level DEBUG`.

---

### Storage

snow-flow stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/snow-flow/`
- **Windows**: `%USERPROFILE%\.local\share\snow-flow`
...
3. Ensure you have the latest version with `snow-flow upgrade`

---

### Authentication issues

1. Try re-authenticating with `snow-flow auth login <provider>`
...
To figure out what models you have access to, run `snow-flow models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/snow-flow
   ```

3. Re-authenticate with your provider:
   ```bash
   snow-flow auth login <provider>
   ```
...
If you encounter API call errors, this may be due to outdated provider packages. snow-flow dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/snow-flow
   ```

2. Restart snow-flow to reinstall the latest provider packages

This will force snow-flow to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
snow-flow will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
