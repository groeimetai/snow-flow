---
title: Server
description: Interact with snow-flow server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `snow-flow serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an snow-flow client can use.
...
```bash
snow-flow serve [--port <number>] [--hostname <string>]
```
...
When you run `snow-flow` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the snow-flow server to interact with snow-flow programmatically.
:::

This architecture lets snow-flow support multiple clients and allows you to interact with snow-flow programmatically.

You can run `snow-flow serve` to start a standalone server. If you have the
snow-flow TUI running, `snow-flow serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Snow-Flow [IDE](/docs/ide) plugins.
...
The snow-flow server exposes the following APIs.
...
| `POST`   | `/session/:id/message`                   | Send chat message                  | body matches [`ChatInput`](https://github.com/groeimetai/snow-flow/blob/main/packages/snow-flow/src/session/index.ts#L358), returns <a href={typesUrl}><code>Message</code></a>     |
| `POST`   | `/session/:id/shell`                     | Run a shell command                | body matches [`CommandInput`](https://github.com/groeimetai/snow-flow/blob/main/packages/snow-flow/src/session/index.ts#L1007), returns <a href={typesUrl}><code>Message</code></a> |
