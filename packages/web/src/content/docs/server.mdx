---
title: Server
description: Interact with snow-code server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `snow-code serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an snow-code client can use.
...
```bash
snow-code serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
snow-code serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `SNOW_CODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `snow-code`, or set `SNOW_CODE_SERVER_USERNAME` to override it. This applies to both `snow-code serve` and `snow-code web`.

```bash
SNOW_CODE_SERVER_PASSWORD=your-password snow-code serve
```
...
When you run `snow-code` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the snow-code server to interact with snow-code programmatically.
:::

This architecture lets snow-code support multiple clients and allows you to interact with snow-code programmatically.

You can run `snow-code serve` to start a standalone server. If you have the
snow-code TUI running, `snow-code serve` will start a new server.
...
The snow-code server exposes the following APIs.
