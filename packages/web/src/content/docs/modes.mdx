...
:::caution
Modes are now configured through the `agent` option in the snow-flow config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in snow-flow allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the snow-flow config.
...
snow-flow comes with two built-in modes.
...
Configure modes in your `.mcp.json` config file:
...
- Global: `~/.config/snow-flow/mode/`
- Project: `.snow-flow/mode/`

```markdown title="~/.config/snow-flow/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global snow-flow config and the project specific config.
...
Create mode files in `.snow-flow/mode/` for project-specific modes or `~/.config/snow-flow/mode/` for global modes:

```markdown title=".snow-flow/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/snow-flow/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
