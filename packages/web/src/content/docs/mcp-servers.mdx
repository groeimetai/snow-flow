...
You can add external tools to Snow-Flow using the _Model Context Protocol_, or MCP. Snow-Flow supports both local and remote servers.
...
You can define MCP servers in your [Snow-Flow Config](https://snow-flow.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="snowcode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/snow-code` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Snow-Flow automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Snow-Flow will:
...
If the server requires authentication, Snow-Flow will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `snow-code mcp auth <server-name>`.
...
```bash
snow-code mcp auth my-oauth-server
```
...
```bash
snow-code mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Snow-Flow will store the tokens securely in `~/.local/share/snow-code/mcp-auth.json`.
...
snow-code mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Snow-Flow, alongside built-in tools. So you can manage them through the Snow-Flow config like any other tool.
...
```json title="snowcode.json" {11, 14-18}
{
  "$schema": "https://snow-flow.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
snow-code mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Snow-Flow to your Sentry account.
