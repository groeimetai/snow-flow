...
You can add external tools to Snow-Flow using the _Model Context Protocol_, or MCP.

Snow-Flow supports both:
...
You can define MCP servers in your Snow-Flow config under `mcp`. Add each MCP
with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title=".mcp.jsonc" {6}
{
...
Your MCPs are available as tools in Snow-Flow, alongside built-in tools. So you
can manage them through the Snow-Flow config like any other tool.
