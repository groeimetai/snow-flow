...
Snow-Flow has a list of keybinds that you can customize through the Snow-Flow config.
...
Snow-Flow uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Snow-Flow desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `snowcode.json`.
