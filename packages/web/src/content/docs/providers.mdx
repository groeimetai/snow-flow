---
title: Providers
description: Using any LLM provider in Snow-Flow.
---

import config from "../../../config.mjs"
export const console = config.console

Snow-Flow uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Snow-Flow config.
...
in `~/.local/share/snow-code/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Snow-Flow
config.
...
Snow-Flow Zen is a list of models provided by the Snow-Flow team that have been
tested and verified to work well with Snow-Flow. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Snow-Flow Zen.
:::

1. Run the `/connect` command in the TUI, select snow-code, and head to [snow-flow.dev/auth](https://snow-flow.dev/auth).
...
It works like any other provider in Snow-Flow and is completely optional to use.
...
To use Amazon Bedrock with Snow-Flow:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running snow-code:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY snow-code

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile snow-code

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX snow-code
   ```
   ...
   For project-specific or persistent configuration, use `snowcode.json`:

   ```json title="snowcode.json"
   {
     "$schema": "https://snow-flow.dev/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in Snow-Flow is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for snow-code to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX snow-code
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX snow-code
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your snow-code config.
...
to store token in snow-code auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Snow-Flow uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Snow-Flow
to only use your own GitLab-hosted instance, add the following to your
`snowcode.json` file. It is also recommended to disable session sharing.
...
More documentation on [snow-code-gitlab-auth](https://www.npmjs.com/package/@gitlab/snow-code-gitlab-auth) homepage.

##### Configuration

Customize through `snowcode.json`:
...
{
  "$schema": "https://snow-flow.dev/config.json",
  "plugin": ["@gitlab/snow-code-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with snow-code:
...
To use Google Vertex AI with Snow-Flow:
...
2. Set the required environment variables:
   ...
   Set them while running snow-code.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id snow-code
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through snow-code, you can always configure it yourself.
...
```jsonc title="~/.config/snow-code/snowcode.jsonc"
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [snow-code-helicone-session](https://github.com/H2Shami/snow-code-helicone-session) plugin to automatically log each Snow-Flow conversation as a session in Helicone.

```bash
npm install -g snow-code-helicone-session
```
...
```json title="snowcode.json"
{
  "plugin": ["snow-code-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Snow-Flow conversation listed as a separate session.
...
You can configure snow-code to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="snowcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure snow-code to use local models through LM Studio.

```json title="snowcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "lmstudio": {
...
You can configure snow-code to use local models through Ollama.

```json title="snowcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Snow-Flow:
...
3. Copy the API key for use in Snow-Flow.
...
6. **Important**: Before using cloud models in Snow-Flow, you must pull the model information locally:
...
Snow-Flow Zen is a list of tested and verified models provided by the Snow-Flow team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Snow-Flow Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Snow-Flow Zen**.

   ```txt
   /connect
   ```

3. Enter your Snow-Flow API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your snow-code config.
...
5. You can also customize them through your snow-code config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' snow-code
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group snow-code
   ```
...
You can also customize models through your snow-code config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with snow-code. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in snowcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `snowcode.json` file in your project directory:

   ```json title="snowcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://snow-flow.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="snowcode.json" {9,11,17-20}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Snow-Flow to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `snow-code auth list` to see if the credentials
...
2. For custom providers, check the snow-code config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your snow-code config.
