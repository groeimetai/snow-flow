...
Snow-Flow uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using `snow-flow auth login`.
...
When you add a provider's API keys with `snow-flow auth login`, they are stored
in `~/.local/share/snow-flow/auth.json`.
...
1. Run `snow-flow auth login`, select snow-flow, and head to [snow-flow.dev/auth](https://snow-flow.dev/auth).
...
3. Paste your API key.
   ...
   │  snow-flow
   │
   ●  Create an api key at https://snow-flow.dev/auth
   │
   ◆  Enter your API key
   │  _
   └
...
1. You'll need either to set one of the following environment variables:
   ...
   Once you have one of the above, set it while running snow-flow.

   ```bash
   AWS_ACCESS_KEY_ID=XXX snow-flow
   ```
...
We recommend signing up for [Claude Pro](https://www.anthropic.com/news/claude-pro) or [Max](https://www.anthropic.com/max), it's the most cost-effective way to use snow-flow.

Once you've signed up, run `snow-flow auth login` and select Anthropic.
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for snow-flow to work properly.
   :::

3. Run `snow-flow auth login` and select **Azure**.
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX snow-flow
   ```
...
2. Run `snow-flow auth login` and select **Cerebras**.
...
2. Run `snow-flow auth login` and select **DeepSeek**.
...
2. Run `snow-flow auth login` and select **Fireworks AI**.
...
To use your GitHub Copilot subscription with snow-flow:
...
1. Run `snow-flow auth login` and select GitHub Copilot.
...
2. Run `snow-flow auth login` and select Groq.
...
2. Set the required environment variables:
   ...
   Set them while running snow-flow.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_VERTEX_PROJECT=your-project-id snow-flow
   ```
...
You can configure snow-flow to use local models through LM Studio.

```json title=".mcp.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "lmstudio": {
...
2. Run `snow-flow auth login` and select **Moonshot AI**.
...
You can configure snow-flow to use local models through Ollama.

```json title=".mcp.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://snow-flow.dev/config.json",
  "provider": {
    "ollama": {
...
2. Run `snow-flow auth login` and select OpenAI.
...
2. Run `snow-flow auth login` and select **Snow-Flow Zen**.
...
2. Run `snow-flow auth login` and select OpenRouter.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your snow-flow config.
...
5. You can also customize them through your snow-flow config. Here's an example of specifying a provider
...
2. Run `snow-flow auth login` and select **Together AI**.
...
For a limited time, you can use xAI's Grok Code for free with snow-flow.

:::tip
Grok Code is available for free for a limited time on snow-flow.
:::

1. Make sure you are on the latest version of snow-flow.
...
2. Run `snow-flow auth login` and select **Z.AI**.
...
To add any **OpenAI-compatible** provider that's not listed in `snow-flow auth login`:

:::tip
You can use any OpenAI-compatible provider with snow-flow. Most modern AI providers offer OpenAI-compatible APIs.
:::

1. Run `snow-flow auth login` and scroll down to **Other**.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need configure it in .mcp.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `.mcp.json` file in your project directory:

   ```json title=".mcp.json" ""myprovider"" {5-15}
   {
     "$schema": "https://snow-flow.dev/config.json",
     "provider": {
       "myprovider": {
...
1. **Check the auth setup**: Run `snow-flow auth list` to see if the credentials
...
2. For custom providers, check the snow-flow config and:
   - Make sure the provider ID used in `snow-flow auth login` matches the ID in your snow-flow config.
