...
// TUI loader for integrated startup
import { ensureTUIBinary, isTUIInstalled } from './tui/index.js';
...
// ASCII Art Banner for Snow-Flow (matching snow-code style with triangles)
function showSnowFlowBanner(): void {
  ...
  // FLOW lines with ░ effect (highlighted)
  const flowLines = [
    '█▀▀▀ █    █▀▀█ █   █',
    '█▀▀░ █    █░░█ █░█░█',
    '▀    ▀▀▀▀ ▀▀▀▀ ▀▀▀▀▀'
  ...
  // Combine side by side with chalk applied PER LINE
  console.log('');
  for (let i = 0; i < snowLines.length; i++) {
    console.log(chalk.dim(triangleLines[i]) + chalk.dim(snowLines[i]) + chalk.white(flowLines[i]));
  }
...
program
  ...
  .action(async (objective: string, options) => {
    ...
    let sessionId = `agent_${Date.now()}_${randomBytes(6).toString('hex')}`;
...
// Start TUI if no command provided, otherwise parse commands
async function main() {
  const args = process.argv.slice(2);

  // If no arguments provided, start TUI with integrated setup
  if (!args.length) {
    await startTUIWithSetup();
    return;
  }

  // Otherwise, parse commands as usual
  program.parse(process.argv);
}

/**
 * Get the snowcode binary path for the current platform
 */
function getSnowcodeBinaryPath(): string | null {
  ...
  const packageName = `@groeimetai/snow-flow-snowcode-${platform}-${arch}`;

  // Look for the binary in various possible locations
  const possiblePaths = [
    // From packages/core/dist/ -> packages/snowcode/dist/
    join(__dirname, '..', '..', 'snowcode', 'dist', packageName, 'bin', `snow-code${extension}`),
    // From packages/core/src/ -> packages/snowcode/dist/
    join(__dirname, '..', 'snowcode', 'dist', packageName, 'bin', `snow-code${extension}`),
    // Monorepo root
...
/**
 * Start TUI with integrated init/auth checks
 */
async function startTUIWithSetup() {
  showSnowFlowBanner();

  try {
    // 1. Check if project is initialized (silent)
    const mcpConfigPath = join(process.cwd(), '.mcp.json');
    if (!existsSync(mcpConfigPath)) {
      // Project not initialized - TUI will handle this
    }

    // 2. Check authentication (silent)
    const authPath = join(os.homedir(), '.local', 'share', 'snow-code', 'auth.json');
    if (!existsSync(authPath)) {
      // Not authenticated - TUI will handle this via /auth command
    }

    // 3. Find snowcode binary (includes HTTP server + TUI)
    ...
    // 4. Start snowcode (includes HTTP server + TUI)
    const snowcodeProcess = spawn(snowcodeBinaryPath, [], {
      stdio: 'inherit',
      env: {
        ...process.env,
      }
    ...
    console.log('');
    console.log(chalk.dim('Falling back to help...'));
    program.outputHelp();
    process.exit(1);
  }
}
