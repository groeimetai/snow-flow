...
export const toolDefinition: MCPToolDefinition = {
  ...
  description: '⚠️ TRIGGERS (not executes directly) an existing scheduled job via sys_trigger. Job runs asynchronously when scheduler picks it up.',
  ...
  try {
    ...
    if (!jobResponse.data?.result) {
      throw new SnowFlowError(
        ErrorType.NOT_FOUND,
        `Scheduled job not found: ${job_sys_id}`,
        { retryable: false }
      );
    }
    ...
    if (!triggerCreated) {
      return createSuccessResult({
        ...
        job_sys_id,
        job_name: job.name,
        message: 'Could not create sys_trigger. You may not have permission to create triggers. Run the job manually via System Scheduler > Scheduled Jobs.',
