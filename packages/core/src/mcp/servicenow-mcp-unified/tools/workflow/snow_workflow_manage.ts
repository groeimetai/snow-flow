    ...
    switch (action) {
      case 'list': {
        ...
        var listSummary = summary()
          .success('Found ' + workflowList.length + ' workflow' + (workflowList.length === 1 ? '' : 's'));
      ...
      case 'stop': {
        ...
        var stopSummary = summary()
          .success('Stopped workflow execution')
          .field('Context ID', context_id);
