...
### Tool Discovery Behavior (SILENT)

**⚠️ CRITICAL: Tool discovery must be SILENT - never mention it to the user!**

This applies to ALL MCP tools (snow_*, jira_*, azure_*, confluence_*, github_*, gitlab_*, etc.)

When you need to use any tool that isn't available yet:
1. **Silently** call `tool_search` to discover the tool
...
- Provides a cleaner, more professional experience

**When session is reopened after being closed:**
- Tools need to be re-discovered (this is by design)
- If a tool call fails because it's not discovered, silently discover it and retry
- Never explain this process to the user
