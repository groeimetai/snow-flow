...
const TIPS = [
  ...
  "Create {highlight}snow-code.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/snow-code/snow-code.json{/highlight} for global config",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.snow-code/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.snow-code/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Snow-Flow auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Snow-Flow uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.snow-code/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.snow-code/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Snow-Flow from reading sensitive files",
  "Use {highlight}snow-code run{/highlight} for non-interactive scripting",
  "Use {highlight}snow-code --continue{/highlight} to resume the last session",
  "Use {highlight}snow-code run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}snow-code serve{/highlight} for headless API access to Snow-Flow",
  "Use {highlight}snow-code run --attach{/highlight} to connect to a running server",
  "Run {highlight}snow-code upgrade{/highlight} to update to the latest version",
  "Run {highlight}snow-code auth list{/highlight} to see all configured providers",
  "Run {highlight}snow-code agent create{/highlight} for guided agent creation",
  "Use {highlight}/snow-code{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}snow-code github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/snow-code fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.snow-code/themes/{/highlight} directory",
  ...
  "Run {highlight}snow-code debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/groeimetai/snow-code{/highlight} for containerized use",
