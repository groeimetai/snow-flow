...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at snow-flow.dev",
  ...
  "Create {highlight}snow-flow.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/snow-flow/snow-flow.json{/highlight} for global config",
  ...
  "Snow-Flow auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.snow-flow/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.snow-flow/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Use Snow-Flow to create Business Rules, Client Scripts, and Script Includes",
  "Snow-Flow includes 54 bundled ServiceNow domain skills for ITSM, CMDB, Security, and more",
  "Snow-Flow has 200+ MCP tools for querying tables, deploying widgets, and managing Update Sets",
  "Use Snow-Flow to query any ServiceNow table with GlideRecord patterns",
  "Create {highlight}.ts{/highlight} files in {highlight}.snow-flow/tool/{/highlight} to define new LLM tools",
  "Ask Snow-Flow to deploy widgets to Service Portal with automatic coherence validation",
  ...
  "Run {highlight}snow-flow serve{/highlight} for headless API access to Snow-Flow",
  "Use {highlight}snow-flow run --attach{/highlight} to connect to a running server",
  "Run {highlight}snow-flow upgrade{/highlight} to update to the latest version",
  "Run {highlight}snow-flow auth list{/highlight} to see all configured providers",
  "Run {highlight}snow-flow agent create{/highlight} for guided agent creation",
  "Use {highlight}/snow-flow{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}snow-flow github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/snow-flow fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.snow-flow/themes/{/highlight} directory",
  "Ask Snow-Flow to create REST integrations and test them against your ServiceNow instance",
  "Use Snow-Flow Enterprise for Jira, Azure DevOps, and Confluence integrations",
  ...
  "Run {highlight}snow-flow debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} with Snow-Flow Enterprise for curated, tested models",
  "Snow-Flow can search your ServiceNow CMDB, manage incidents, and automate workflows",
