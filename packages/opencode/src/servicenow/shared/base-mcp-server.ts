...
export abstract class BaseMCPServer {
  ...
  private registerPromptHandlers(): void {
    ...
    this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
      ...
      const result = await this.promptManager.executePrompt(name, (args as Record<string, string>) || {})
...
💡 To get OAuth credentials:
   ...
   */
  protected async trackArtifact(sysId: string, type: string, name: string, updateSetId?: string): Promise<void> {
    try {
      if (!updateSetId) {
   ...
   */
  protected async reportProgress(context: AgentContext, progress: number, phase: string): Promise<void> {
  ...
  private categorizeError(errorMessage: string, errorCode?: string): string {
    ...
    // Permission errors
    if (
      msgLower.includes("insufficient privileges") ||
    ...
    // Scope errors
    if (msgLower.includes("scope") || msgLower.includes("application") || msgLower.includes("global")) {
      return "scope"
    }
  ...
  private async handleNetworkError(
    ...
    return this.createErrorResponse(`Network error during ${operation}`, {
      ...
      next_steps: ["Wait for network recovery and retry", "Or check ServiceNow instance status"],
    })
  }
   ...
   */
  private async handleValidationError(error: any, operation: string, context: AgentContext): Promise<MCPToolResult> {
    const errorMessage = error instanceof Error ? error.message : String(error)
    ...
    return this.createErrorResponse(`Validation error during ${operation}`, {
      ...
      next_steps: ["Update configuration with correct field values", "Validate against ServiceNow table requirements"],
    })
  }
  ...
  private async extractEnhancedDeploymentContext(operation: string, context: AgentContext): Promise<any> {
    try {
      ...
      const artifactName =
        contextData.artifact_name || this.extractArtifactName(operation) || `fallback_${artifactType}_${Date.now()}`
  ...
  private async deployGenericArtifactInGlobalScope(context: any): Promise<any> {
    try {
      ...
      const possibleTables = ["sys_script_include", "sys_script", "sys_ui_script", "sys_ui_page"]
