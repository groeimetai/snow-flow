    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        // Knowledge Management Tools
        {
          ...
          description:
            "Creates a knowledge article in ServiceNow Knowledge Base. Articles can contain solutions, how-to guides, or reference information.",
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          description:
            "Searches knowledge articles using keywords, categories, or filters. Returns relevant articles with snippets.",
          inputSchema: {
            type: "object",
            properties: {
        ...
        // Service Catalog Tools
        {
          ...
          description:
            "Creates a service catalog item for user self-service requests. Includes forms, workflows, and fulfillment.",
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              type: {
                type: "string",
                description: "Type: single_line_text, multi_line_text, select_box, checkbox, reference, etc.",
              },
                ...
                items: {
                  ...
                  properties: {
                    ...
                    operation: {
                      type: "string",
                      description:
                        "Comparison operation: is, is_not, is_empty, is_not_empty, contains, does_not_contain",
                      default: "is",
                    },
  ...
  private async createCatalogUIPolicy(args: any) {
    ...
    try {
      ...
      const resolveVariableId = async (variableName: string, catalogItem: string): Promise<string> => {
          ...
          try {
            ...
            if (varResponse.success && varResponse.data.result.length > 0) {
              ...
              if (strategy.description === "all variables for manual matching") {
                const match = varResponse.data.result.find(
                  (v: any) =>
                    v.name === variableName ||
        ...
        this.logger.error(
          `❌ CRITICAL: Variable '${variableName}' not found in any table for catalog item ${catalogItem}`,
        )
      ...
      if (args.conditions && Array.isArray(args.conditions)) {
        ...
        for (const condition of args.conditions) {
          ...
          if (!isValidSysId) {
            this.logger.warn(
              `⚠️ Variable '${condition.catalog_variable}' could not be resolved to sys_id - condition may fail!`,
            )
          }
        ...
        for (let i = 0; i < args.actions.length; i++) {
          ...
          if (actionResponse.success) {
            ...
            if (!actualUiPolicyId || actualUiPolicyId === "" || actualUiPolicyId === "{}") {
              ...
              throw new Error(
                `Action ${i + 1} created but ui_policy field is empty - action will not work! This may be a ServiceNow API or table structure issue.`,
              )
            }

            // For reference fields, ServiceNow might return an object - extract the value
            const uiPolicySysId =
              typeof uiPolicyValue === "object" && uiPolicyValue.value ? uiPolicyValue.value : uiPolicyValue
            ...
            const catalogItemSysId =
              typeof catalogItemValue === "object" && catalogItemValue.value ? catalogItemValue.value : catalogItemValue
      ...
      if (!itemResponse.success) {
        ...
        if (!itemResponse.success || itemResponse.data.result.length === 0) {
          ...
          if (queryResponse.success && queryResponse.data.result.length > 0) {
            ...
            throw new Error(
              `Catalog item not found with sys_id: ${args.sys_id}. Tried getRecord, searchRecords, and queryTable.`,
            )
          }
        } else {
...
🔄 Active: ${item.active ? "Yes" : "No"}`
      ...
      if (args.include_variables) {
        ...
        if (varResponse.success && varResponse.data.result.length) {
          const variables = varResponse.data.result
            .map((v: any) => `  - ${v.question_text} (${v.type})${v.mandatory ? " *Required" : ""}`)
            .join("\n")
          details += `\n\n📋 **Variables:**\n${variables}`
        }
      }
  ...
  private async discoverCatalogs(args: any) {
    try {
      ...
      const catalogText = catalogsWithDetails
        .map((catalog: any) => {
