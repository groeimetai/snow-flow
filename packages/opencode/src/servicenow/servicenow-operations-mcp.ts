...
class ServiceNowOperationsMCP {
      ...
      return {
        tools: [
          // 🎯 UNIVERSAL TABLE QUERY - Works for ANY ServiceNow table!
          {
            ...
            description:
              "Universal query tool for any ServiceNow table. SMART ANALYTICS: OMIT limit for ALL records with minimal fields. For display use limit:50. For counting use include_content:false. See CLAUDE.md for optimal query patterns.",
            inputSchema: {
              type: "object",
              properties: {
                table: {
                  type: "string",
                  description:
                    "ServiceNow table name (e.g., incident, sc_request, problem, task, change_request, u_custom_table)",
                ...
                limit: {
                  type: "number",
                  description:
                    "Maximum records to return. OMIT for analytics (gets ALL records). Use 10-50 for display, 5000+ for ML training. No default - system auto-detects best approach.",
                  examples: [50, 1000, 5000],
                },
                include_content: {
                  type: "boolean",
                  description:
                    "Include full record data. Default false for performance, returns count only. Set true for detailed results.",
                  default: false,
                },
                fields: {
                  type: "array",
                  items: { type: "string" },
                  description:
                    'Specific fields to return (automatically sets include_content=true). Examples: ["number", "short_description", "state"]',
                },
...

          {
            ...
            description:
              "Searches knowledge base articles using intelligent matching algorithms. Links articles to incidents for resolution guidance.",
            inputSchema: {
              type: "object",
              properties: {
                query: {
          ...
          // Service Catalog Management
          {
            ...
            description:
              "Comprehensive service catalog management including item creation, variable configuration, workflow attachment, and lifecycle management.",
            inputSchema: {
              type: "object",
              properties: {
                action: {
...

          {
            ...
            description:
              "Searches service catalog items with fuzzy matching and category filtering. Returns items with full variable and pricing details.",
            inputSchema: {
              type: "object",
              properties: {
                query: {
...

          {
            ...
            description:
              "Removes test artifacts (Test*, Mock*, Demo*) from ServiceNow while preserving Update Set history for audit compliance.",
            inputSchema: {
              type: "object",
              properties: {
                artifact_types: {
          ...
          // User and Group Management Tools
          {
            ...
            description:
              "Creates user groups in ServiceNow with configurable hierarchy, managers, and types for access control and assignment management.",
            inputSchema: {
              type: "object",
              properties: {
                name: {
...

          {
            ...
            description:
              "Creates new users in ServiceNow with complete profile information including department, manager, and initial credentials.",
            inputSchema: {
              type: "object",
              properties: {
                user_name: {
...

          {
            ...
            description:
              "Assigns users to groups for role-based access control and workflow assignments. Supports lookup by sys_id or username.",
            inputSchema: {
              type: "object",
              properties: {
                user: {
...

          {
            ...
            description:
              "Removes users from groups to revoke access and update role assignments. Maintains audit trail of membership changes.",
            inputSchema: {
              type: "object",
              properties: {
                user: {
...

          {
            ...
            description:
              "Lists all members of a specified group with filtering for active/inactive users. Returns user details and roles.",
            inputSchema: {
              type: "object",
              properties: {
                group: {
  ...
  private async handleUniversalQuery(args: any) {
    ...
    const determineSmartLimit = (
      ...
      this.logger.info(
        `⚠️ No specific context detected - using conservative limit. Consider specifying limit for your use case!`,
      )
    ...
    if (isMLTrainingContext && limit < 1000) {
      this.logger.warn(
        `⚠️  ML Training detected with low limit (${limit}). Consider setting limit=5000+ for better training data!`,
      )
    }

    this.logger.info(
      `Universal query on table '${table}' with: ${query} (limit: ${limit === undefined ? "UNLIMITED" : limit}, offset: ${offset}, include_content: ${include_content})`,
    )
    ...
    try {
      ...
      if (shouldIncludeContent) {
        ...
        if (fields && fields.length > 0 && records.success && records.data.result.length > 0) {
          ...
          const validFields = fields.filter((field: string) => {
            const exists = sampleRecord.hasOwnProperty(field) || sampleRecord.hasOwnProperty(`${field}_display_value`)
            if (!exists) {
      ...
      if (error.code === "ECONNREFUSED") {
        throw new McpError(
          ...
          `Cannot connect to ServiceNow. Please check your instance URL and network connection.`,
        )
      }
      ...
      if (error.response?.status === 400) {
        throw new McpError(
          ErrorCode.InvalidParams,
          `Invalid query syntax: ${query}. Check ServiceNow encoded query format.`,
        )
      }
  ...
  private async handleKnowledgeSearch(args: any) {
    ...
    try {
      ...
      if (match_incident) {
        ...
        if (incident) {
          const relevanceScores = await this.calculateKnowledgeRelevance(
            articles.success ? articles.data.result : [],
            incident,
          )
  ...
  private async performIncidentAnalysis(
    incident: any,
    include_similar: boolean,
    suggest_resolution: boolean,
  ...
  private async getCIRelationships(configItems: any[]): Promise<any[]> {
    ...
    for (const ci of configItems) {
      try {
        const ciRelationships = await this.client.searchRecords(
          "cmdb_rel_ci",
          `parent=${ci.sys_id}^ORchild=${ci.sys_id}`,
          10,
        )
        relationships.push({
          ci_id: ci.sys_id,
  ...
  private async calculateOperationalMetrics(timeframe: string, metricTypes: string[]): Promise<OperationalMetrics> {
    ...
    try {
      ...
      const highPriorityIncidents = await this.client.searchRecords(
        "incident",
        `priority=1^sys_created_on${dateFilter}`,
        1000,
      )
      ...
      problems.data.result.forEach((problem: any) => {
        ...
        if (problem.resolution_notes) {
          ...
          if (resolution.includes("configuration")) {
            rootCauses.resolution_patterns.set(
              "configuration",
              (rootCauses.resolution_patterns.get("configuration") || 0) + 1,
            )
          }
          if (resolution.includes("patch") || resolution.includes("update")) {
            rootCauses.resolution_patterns.set(
              "patch/update",
              (rootCauses.resolution_patterns.get("patch/update") || 0) + 1,
            )
          }
        }
      })
    ...
    try {
      ...
      switch (predictionType) {
        ...
        case "user_impact":
          ...
          break
        default:
          _analysis.data_points = [
            {
              error: "Unknown prediction type",
              supported_types: ["incident_volume", "system_failure", "resource_exhaustion", "user_impact"],
            },
          ]
  ...
  private async analyzeIncidentTrends(timeframe: string): Promise<any[]> {
    try {
      const incidents = await this.client.searchRecords(
        "incident",
        "sys_created_onONToday@javascript:gs.daysAgoStart(7)@javascript:gs.daysAgoEnd(0)",
        100,
      )
  ...
  private async analyzeSystemHealthTrends(timeframe: string): Promise<any[]> {
    try {
      const criticalIncidents = await this.client.searchRecords(
        "incident",
        "priority=1^sys_created_onONToday@javascript:gs.daysAgoStart(7)@javascript:gs.daysAgoEnd(0)",
        50,
      )
  ...
  private async analyzeUserImpactTrends(timeframe: string): Promise<any[]> {
    try {
      const userIncidents = await this.client.searchRecords(
        "incident",
        "caller_idISNOTEMPTY^sys_created_onONToday@javascript:gs.daysAgoStart(7)@javascript:gs.daysAgoEnd(0)",
        100,
      )
  ...
  private async predictIncidentVolume(timeframe: string): Promise<any[]> {
    ...
    const historicalData = await this.client.searchRecords(
      "incident",
      "sys_created_onONLast 30 days@javascript:gs.daysAgoStart(30)@javascript:gs.daysAgoEnd(0)",
      1000,
    )
  ...
  private async predictSystemFailure(timeframe: string): Promise<any[]> {
    ...
    const problems = await this.client.searchRecords(
      "problem",
      "sys_created_onONLast 30 days@javascript:gs.daysAgoStart(30)@javascript:gs.daysAgoEnd(0)",
      100,
    )

    const systemFailures = problems.success
      ? problems.data.result.filter(
          (problem: any) =>
  ...
  private async predictUserImpact(timeframe: string): Promise<any[]> {
    ...
    const userIncidents = await this.client.searchRecords(
      "incident",
      "sys_created_onONLast 7 days@javascript:gs.daysAgoStart(7)@javascript:gs.daysAgoEnd(0)^priority=1",
      100,
    )
  ...
  private async handleCatalogItemSearch(args: any) {
    ...
    try {
      ...
      if (category_filter) {
        ...
        const categoryResult = await this.client.searchRecords(
          "sc_category",
        ...
        if (categoryResult.success && categoryResult.data?.result?.length > 0) {
          ...
          const exactMatch = categoryResult.data.result.find(
            (c: any) => c.name.toLowerCase() === category_filter.toLowerCase(),
          )
  ...
  private shouldExcludeItem(query: string, itemName: string, itemDesc: string): boolean {
    ...
    if (
      ...
      (itemName.includes("decommission") ||
        itemName.includes("service") ||
        itemName.includes("process") ||
        itemName.includes("request") ||
        itemName.includes("removal") ||
        itemName.includes("decomm") ||
        itemDesc.includes("decommission") ||
        itemDesc.includes("service process"))
    ) {
      return true
    }
    ...
    if (lowerQuery.includes("monitor") || lowerQuery.includes("display") || lowerQuery.includes("screen")) {
      ...
      if (
        (itemName.includes("server") ||
          ...
          itemName.includes("software") ||
          itemName.includes("license") ||
          itemName.includes("training") ||
          itemName.includes("support")) &&
        !itemName.includes("monitor") &&
        !itemName.includes("display") &&
  ...
  private async handleCleanupTestArtifacts(args: any) {
    ...
    try {
      ...
      // Generate summary
      const totalFound = Object.values(cleanupResults.artifacts_found).reduce(
        (sum, items) => (typeof sum === "number" ? sum : 0) + (Array.isArray(items) ? items.length : 0),
        0,
      )
      const totalDeleted = Object.values(cleanupResults.artifacts_deleted).reduce(
