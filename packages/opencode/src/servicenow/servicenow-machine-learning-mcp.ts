...
if (typeof global !== "undefined") {
  ...
  if (!global.performance || !global.performance.now) {
    ;(global as any).performance = {
      ...
      getEntriesByName: perfHooksPerformance.getEntriesByName
        ? perfHooksPerformance.getEntriesByName.bind(perfHooksPerformance)
        : () => [],
      getEntriesByType: perfHooksPerformance.getEntriesByType
        ? perfHooksPerformance.getEntriesByType.bind(perfHooksPerformance)
        : () => [],
      clearMarks: perfHooksPerformance.clearMarks
        ? perfHooksPerformance.clearMarks.bind(perfHooksPerformance)
        : () => {},
      clearMeasures: perfHooksPerformance.clearMeasures
        ? perfHooksPerformance.clearMeasures.bind(perfHooksPerformance)
        : () => {},
...
export class ServiceNowMachineLearningMCP {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      const tools: Tool[] = [
        // Training tools
        {
          ...
          description:
            "Trains LSTM neural networks on historical incident data with intelligent data selection. Automatically optimizes dataset size up to 5000 records for best accuracy. Works without PA/PI licenses.",
          inputSchema: {
            type: "object",
            properties: {
              sample_size: {
                type: "number",
                description:
                  "Number of incidents to use for training. If not specified, automatically uses all available data (up to 5000). Set to limit training data.",
              },
              auto_maximize_data: {
                type: "boolean",
              ...
              query: {
                type: "string",
                description:
                  "Custom ServiceNow query for selecting training data. If not provided, Snow-Flow will intelligently select data.",
                default: "",
              },
              intelligent_selection: {
                type: "boolean",
                description:
                  "Let Snow-Flow intelligently select balanced training data across categories, priorities, and time periods. Combined with auto_maximize_data for optimal results.",
                default: true,
              },
              focus_categories: {
                type: "array",
        ...
        },
        {
          ...
          description:
            "Trains neural networks to predict change implementation risks based on historical change data. Works without PA/PI licenses.",
          inputSchema: {
            type: "object",
            properties: {
              sample_size: {
        ...
        },
        {
          ...
          description:
            "Trains autoencoder neural networks for anomaly detection in system metrics. Works without PA/PI licenses using standard table data.",
          inputSchema: {
            type: "object",
            properties: {
              metric_type: {
        ...
        },
        {
          ...
          description:
            "Detects anomalies in incident patterns, user behavior, or system performance using autoencoder models.",
          inputSchema: {
            type: "object",
            properties: {
              metric_type: {
        ...
        // Model management
        {
          ...
          description:
            "Retrieves status and performance metrics for all trained ML models including accuracy, loss, and usage statistics.",
          inputSchema: {
            type: "object",
            properties: {
              model: {
        ...
        },
        {
          ...
          description:
            "Evaluates model performance using test datasets. Returns accuracy, precision, recall, and F1 scores.",
          inputSchema: {
            type: "object",
            properties: {
              model: {
        ...
        },
        {
          ...
          description:
            "Uses ServiceNow Predictive Intelligence for high-accuracy incident classification. Requires Predictive Intelligence plugin license.",
          inputSchema: {
            type: "object",
            properties: {
              operation: {
        ...
        },
        {
          ...
          description:
            "Uses Agent Intelligence for automated work assignment and routing. Requires Agent Intelligence plugin license.",
          inputSchema: {
            type: "object",
            properties: {
              task_type: {
        ...
        },
        {
          ...
          description:
            "Performs ML-driven process optimization and bottleneck analysis. Requires Performance Analytics plugin license.",
          inputSchema: {
            type: "object",
            properties: {
              process_name: {
        ...
        },
        {
          ...
          description:
            "Provides Natural Language Understanding for intent and entity extraction. Requires Virtual Agent plugin license.",
          inputSchema: {
            type: "object",
            properties: {
              text: {
        ...
        },
        {
          ...
          description:
            "Hybrid ML approach that automatically selects between native ServiceNow ML (if licensed) or TensorFlow.js for optimal results.",
          inputSchema: {
            type: "object",
            properties: {
              use_case: {
    ...
    try {
      ...
      if (auto_maximize_data || !sample_size) {
        ...
        try {
          ...
          const countQuery =
            query ||
            (intelligent_selection ? "categoryISNOTEMPTY^descriptionISNOTEMPTY^sys_created_onONLast 6 months" : "")
      ...
      if (this.hasPI) {
        try {
          ...
          const piResult = await this.makeServiceNowRequest(
            "/api/sn_ind/clustering/train",
            {
              table: "incident",
      ...
      this.logger.info(
        `Settings: batch_size=${batch_size}, max_vocabulary=${max_vocabulary_size}, streaming=${streaming_mode}`,
      )
    ...
    try {
      ...
      if (this.hasPI) {
        try {
          ...
          const piResult = await this.makeServiceNowRequest(
            "/api/sn_ind/similar_incident/classify",
            {
      ...
      const confidenceLevel =
        predictions[0].probability > 0.8 ? "High" : predictions[0].probability > 0.6 ? "Medium" : "Low"
            ...
            text: JSON.stringify(
              {
                ...
                incident_details: {
                  ...
                  short_description:
                    incidentData.short_description.substring(0, 100) +
                    (incidentData.short_description.length > 100 ? "..." : ""),
                  analyzed_text_length:
                    (incidentData.short_description + " " + incidentData.description).length + " characters",
                },
                model_info: {
                  method: "tensorflow_js_lstm",
  ...
  private suggestPriority(category: string, description: string): number {
    ...
    if (category.toLowerCase().includes("hardware") || category.toLowerCase().includes("network")) {
      return 2
    }
  ...
  private async fetchIncidentData(
    ...
    try {
      ...
      this.logger.info(
        `🔍 Used fields: ${result.fields.slice(0, 10).join(", ")}${result.fields.length > 10 ? "..." : ""}`,
      )
      ...
      return result.data.map((inc: any) => ({
        ...
        resolution_time:
          inc.resolved_at && inc.sys_created_on
            ? (new Date(inc.resolved_at).getTime() - new Date(inc.sys_created_on).getTime()) / 1000
            : undefined,
      }))
    } catch (error: any) {
  ...
  private async prepareIncidentData(incidents: IncidentData[]) {
    ...
    this.logger.info(
      `Prepared training data: features [${paddedSequences.length}, ${maxLength}], labels [${incidents.length}, ${categories.length}]`,
    )
  ...
  private generateVolumeRecommendations(
    forecast: Float32Array | Int32Array | Uint8Array,
    ...
    if (maxForecast > historicalAvg * 1.5) {
      recommendations.push(
        `Peak expected on day ${Array.from(forecast).indexOf(maxForecast) + 1}. Prepare escalation procedures.`,
      )
    }
  ...
  private async fetchChangeData(limit: number, includeFailed: boolean): Promise<ChangeData[]> {
    ...
    try {
      ...
      if (!response || !response.result) {
        throw new Error(
          "Failed to fetch change data from ServiceNow. " + "Ensure you have permission to read change_request table.",
        )
      }
  ...
  private async fetchMetricData(metricType: string, days: number): Promise<number[][]> {
    ...
    switch (metricType) {
      ...
      default:
        throw new Error(
          `Unsupported metric type: ${metricType}. Supported types: incident_volume, change_volume, request_volume`,
        )
    }
  ...
  private processBatchWithHashing(incidents: any[], categories: string[], hasher: Function) {
    ...
    this.logger.info(
      `Creating tensors: features [${sequences.length}, ${sequences[0]?.length}], labels [${labels.length}, ${labels[0]?.length}]`,
    )
    ...
    try {
      ...
      if (!recommendations.result || recommendations.result.length === 0) {
        ...
        if (task.result && task.result.assignment_group) {
          ...
          recommendations.result = {
            recommendations:
              groupMembers.result?.map((member: any) => ({
      ...
      if (auto_assign && recommendations.result?.top_recommendation) {
        ...
        await this.makeServiceNowRequest(
          `/api/now/table/${task_type}/${task_id}`,
          {
  ...
  private async hybridRecommendation(args: any) {
    ...
    try {
      ...
      const hybridScore = {
        ...
        combined_confidence:
          (nativeResult?.confidence || 0) * native_weight + (customResult?.confidence || 0) * custom_weight,
      }
  ...
  private async makeServiceNowRequest(endpoint: string, params: any, method: string = "GET"): Promise<any> {
    try {
      ...
      const mlAPIEndpoints = ["/api/now/pa/", "/api/sn_ind/", "/api/now/ml/", "/api/now/agent_intelligence/"]
  ...
  private cacheEmbedding(key: string, embedding: tf.Tensor): void {
    ...
    if (this.embeddingCache.size >= ServiceNowMachineLearningMCP.MAX_CACHED_EMBEDDINGS) {
      const keysToRemove = Array.from(this.embeddingCache.keys()).slice(
        0,
        Math.floor(ServiceNowMachineLearningMCP.MAX_CACHED_EMBEDDINGS / 2),
      )
