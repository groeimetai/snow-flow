...
export class ServiceNowClient {
  ...
  constructor() {
    ...
    const httpsAgent = new https.Agent({
      ...
      checkServerIdentity: (hostname, cert) => {
        ...
        const validHosts = [/.*\.service-now\.com$/, /.*\.servicenow\.com$/]

        // Get configured instance hostname
        const instanceUrl = process.env.SNOW_INSTANCE
        if (instanceUrl) {
          try {
            const instanceHostname = new URL(instanceUrl.startsWith("http") ? instanceUrl : `https://${instanceUrl}`)
              .hostname
  ...
  private async handleTokenRefreshWithLock(originalRequest: any): Promise<any> {
    ...
    if (now - this.lastTokenRefresh < 5000) {
      // 5 second rate limit
  ...
  async validateDeploymentPermissions(): Promise<ServiceNowAPIResponse<any>> {
        ...
        test: async () => {
          ...
          try {
            ...
            if (
              ...
              !createResponse.data.result.sys_id
            ) {
              throw new Error(
                "Widget creation succeeded but response structure is unexpected - unable to verify sys_id",
              )
            }
            ...
            try {
              ...
              this.logger.warn(
                `Test widget created but cleanup failed: ${deleteError instanceof Error ? deleteError.message : String(deleteError)}`,
              )
      ...
      },
      {
        ...
        test: async () => {
          const response = await this.client.get(
            `${this.getBaseUrl()}/api/now/table/sys_user_role?sysparm_query=user=javascript:gs.getUserID()`,
          )
          ...
          if (!response || !response.data || !response.data.result) {
            return {
              success: true,
              data: { roles: [], warning: "Unable to retrieve user roles - response structure unexpected" },
            }
          }
    ...
    const failedTests =
      diagnostics.tests && typeof diagnostics.tests === "object"
      ...
      if (error && typeof error === "string") {
        if (error.includes("403")) {
          recommendations.push(
            "🔐 Widget deployment failed with 403 Forbidden. Check OAuth scopes: ensure 'useraccount' and 'glide_system_administration' scopes are enabled",
          )
    ...
    if (roleTest?.status && typeof roleTest.status === "string" && roleTest.status.includes("PASS")) {
      ...
      if (Array.isArray(roles)) {
        ...
        const hasRequiredRole = roles.some((role: any) => {
          if (typeof role === "string") {
            ...
            const isPortalManager =
              roleLower.includes("sp_portal_manager") ||
        ...
        if (!hasRequiredRole) {
          ...
          recommendations.push(
            `⚠️ User roles (${roles.join(", ")}) don't include admin or portal management roles. Contact ServiceNow admin to assign appropriate roles`,
          )
        } else {
  ...
  async updateWidget(
    sysId: string,
    widget: Partial<ServiceNowWidget>,
  ): Promise<ServiceNowAPIResponse<ServiceNowWidget>> {
    try {
...
`
      ...
      for (let i = 0; i < 15; i++) {
        ...
        try {
          const propResponse = await this.client.get(`${this.getBaseUrl()}/api/now/table/sys_properties`, {
      ...
      if (result) {
        ...
        return {
          ...
          data: {
            message:
              "Scheduled job created but execution not confirmed. The sys_trigger may not have been created (permissions) or the scheduler has not yet picked it up.",
  ...
  async createFlow(flow: any): Promise<ServiceNowAPIResponse<any>> {
    try {
      ...
      if (activitiesToProcess.length === 0 && flow.flow_definition) {
        try {
          const flowDef =
            typeof flow.flow_definition === "string" ? JSON.parse(flow.flow_definition) : flow.flow_definition
  ...
  async createSubflow(subflow: any): Promise<ServiceNowAPIResponse<any>> {
    try {
      ...
      if (subflow.flow_definition) {
        const definition =
          typeof subflow.flow_definition === "string" ? JSON.parse(subflow.flow_definition) : subflow.flow_definition
