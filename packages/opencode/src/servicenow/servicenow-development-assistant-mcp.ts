...
export class ServiceNowDevelopmentAssistantMCP {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
          ...
          inputSchema: {
            ...
            properties: {
              query: {
                type: "string",
                description:
                  'Natural language edit instruction (e.g., "pas de flow aan met de naam approval request flow en zorg dat er na de approval stap een mailtje naar test@admin.nl wordt gestuurd")',
              },
            },
            required: ["query"],
          },
        ...
        },
        {
          ...
          description:
            "Searches across multiple ServiceNow tables simultaneously to find artifacts. Includes inactive records and cross-table relationships.",
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          description:
            "Validates multiple artifacts before deployment. Checks dependencies, identifies conflicts, and provides remediation recommendations.",
          inputSchema: {
            type: "object",
            properties: {
              artifacts: {
                ...
                items: {
                  type: "object",
                  properties: { type: { type: "string" }, sys_id: { type: "string" }, table: { type: "string" } },
                },
                description: "List of artifacts to validate",
              },
              validation_level: {
                type: "string",
                enum: ["syntax", "dependencies", "full"],
        ...
        },
        {
          ...
          description:
            "Requests temporary elevated permissions for development operations. Manages role requirements and provides audit trail.",
          inputSchema: {
            type: "object",
            properties: {
              required_roles: {
                type: "array",
                items: { type: "string" },
                description: "Required roles (admin, app_creator, system_administrator)",
              },
              duration: {
                type: "string",
                enum: ["session", "temporary", "workflow"],
              ...
              workflow_context: {
                type: "string",
                description: "Context of the development workflow requiring elevation",
              },
            },
      ...
      try {
        ...
        switch (name) {
          ...
          case "snow_implement_cost_optimization":
            const implementResult = await this.costOptimizationEngine.implementOptimization(
              String(args.optimization_id),
            )
            result = {
              content: [
                {
  ...
  private async findArtifact(args: any) {
    ...
    try {
      ...
      this.logger.info(
        `🔍 Searching ServiceNow with retry logic for: ${intent.identifier} (type: ${intent.artifactType})`,
      )
  ...
  private async comprehensiveSearch(args: any) {
    ...
    try {
      ...
      const uniqueResults = allResults.filter(
        (result, index, self) => index === self.findIndex((r) => r.sys_id === result.sys_id),
      )
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          this.logger.info(
            `🔄 No results found, waiting ${delay}ms before retry (ServiceNow indexes may be updating...)`,
          )
          await this.sleep(delay)
  ...
  private async broadFallbackSearch(intent: ParsedIntent): Promise<any[]> {
    ...
    try {
      ...
      const uniqueResults = broadResults.filter(
        (result, index, self) => index === self.findIndex((r) => r.sys_id === result.sys_id),
      )
   ...
   */
  async searchForRecentlyCreatedArtifact(
    artifactName: string,
    artifactType: string,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (results && results.length > 0) {
          this.logger.info(
            `✅ SNOW-002 RESOLVED: Found ${results.length} results for newly created artifact on attempt ${attempt}`,
          )
          return results
        }
  ...
  private async searchServiceNow(intent: ParsedIntent) {
    try {
      ...
      if (intent.artifactType === "any") {
        ...
        if (
          searchString.toLowerCase().includes("list all") ||
      ...
      if (!results || !results.success || results.data.result.length === 0) {
        ...
        results = await this.client.searchRecords(
          table,
          `short_descriptionLIKE${searchString}^ORdescriptionLIKE${searchString}^LIMIT10`,
        )
      }
  ...
  private async deployArtifact(artifact: any) {
    ...
    try {
      ...
      switch (artifact.type) {
        ...
        case "flow":
          ...
          updateData = {
            ...
            flow_definition:
              typeof artifact.flow_definition === "string"
        ...
        case "subflow":
          ...
          updateData = {
            name: artifact.name,
            description: artifact.description,
            inputs: typeof artifact.inputs === "string" ? artifact.inputs : JSON.stringify(artifact.inputs || []),
            outputs: typeof artifact.outputs === "string" ? artifact.outputs : JSON.stringify(artifact.outputs || []),
          }
          break
      ...
      if (result.result) {
        ...
        const baseUrl =
          (instanceInfo.result && typeof instanceInfo.result === "object" && "instance_url" in instanceInfo.result
            ? instanceInfo.result.instance_url
            : null) || `https://${process.env.SNOW_INSTANCE?.replace(/\/$/, "") || "instance"}.service-now.com`
    ...
    try {
      ...
      if (estimatedTokens > maxTokens) {
        ...
        responseText = `✅ Found artifact by sys_id!\n\n🎯 **${formattedArtifact.name}**\n🆔 sys_id: ${args.sys_id}\n📊 Table: ${args.table}\n\n⚠️ **Response size limited (${estimatedTokens} tokens > ${maxTokens} limit)**\n\n**Essential Fields:**\n${JSON.stringify(essentialFields, null, 2)}\n\n🎯 **RECOMMENDATION: Get specific fields instead**\n\nUse snow_query_table with specific fields for better performance:\n\`\`\`\nsnow_query_table({\n  table: "${args.table}",\n  query: "sys_id=${args.sys_id}",\n  fields: ["field1", "field2", "field3"],  // Only fields you need\n  limit: 1\n})\n\`\`\`\n\n**Available fields to choose from:**\n${availableFields.slice(0, 20).join(", ")}${availableFields.length > 20 ? `, and ${availableFields.length - 20} more...` : ""}\n\n💡 **Example for ${args.table}:**\n${
          args.table === "sp_widget"
            ? `snow_query_table({ table: "sp_widget", query: "sys_id=${args.sys_id}", fields: ["name", "title", "template", "client_script", "script"], limit: 1 })`
            : args.table === "wf_workflow"
              ? `snow_query_table({ table: "wf_workflow", query: "sys_id=${args.sys_id}", fields: ["name", "title", "workflow_version", "stage"], limit: 1 })`
...
3. Use mock testing if record access fails`
  ...
  private async orchestrateDevelopment(args: any) {
    ...
    try {
      const orchestration: any = {
        ...
        warning: auto_deploy
          ? "⚠️ WARNING: This will create REAL artifacts in ServiceNow!"
          : "✅ SAFE: This is planning only, no artifacts will be created",
        agents_spawned: [],
            ...
            try {
              ...
              if (artifact.type === "flow") {
                ...
                deploymentResult = await this.client.post("/api/now/table/sp_widget", {
                  ...
                  client_script: artifact.client_script || "",
                  server_script:
                    artifact.server_script || '(function() { data.message = "Successfully deployed!"; })()',
                  public: true,
                  has_preview: true,
                })
              }
  ...
  private async resilientDeployment(args: any) {
    ...
    try {
      ...
      const successCount = deployment.deployment_results.filter(
        (r: any) => r.status === "deployed" || r.status === "deployed_with_fallback",
      ).length

      deployment.status =
        successCount === artifacts.length ? "completed" : successCount > 0 ? "partially_completed" : "failed"
  ...
  private async detectUpdateSetConflicts(): Promise<any[]> {
    ...
    return [{ conflict_type: "naming", severity: "warning", description: "Similar named artifacts found" }]
  }
      ...
      switch (strategy) {
        ...
        case "simplified_version":
          ...
          if (artifact.type === "flow") {
            ...
            const flowDef =
              typeof artifact.config.flow_definition === "string"
...
try {
  ...
  private async analyzeArtifactRequirements(objective: string): Promise<any> {
    ...
    if (
      lowerObjective.includes("flow") ||
      lowerObjective.includes("workflow") ||
      lowerObjective.includes("automation")
    ) {
    ...
    if (
      lowerObjective.includes("widget") ||
      lowerObjective.includes("dashboard") ||
      lowerObjective.includes("display")
    ) {
      artifactAnalysis.recommended_artifacts.push({
        type: "widget",
        ...
        template:
          '<div class="panel panel-default"><div class="panel-body">{{data.message || "Widget deployed successfully"}}<br><small>Created for: ' +
          objective +
          "</small></div></div>",
        css: ".panel { margin: 10px; }",
        client_script: 'function() { console.log("Widget loaded"); }',
        server_script:
          '(function() { data.message = "Successfully deployed for: ' +
          objective
            .replace(/\\/g, "\\\\")
            .replace(/"/g, '\\"')
            .replace(/'/g, "\\'")
    ...
    if (
      lowerObjective.includes("script") ||
      lowerObjective.includes("function") ||
      lowerObjective.includes("utility")
    ) {
      artifactAnalysis.recommended_artifacts.push({
        type: "script_include",
...
${i + 1}. ${s.title} (${s.impact} impact, ${s.effort} effort)
   ...
   ${s.automated ? "✅ Can be automated" : "⚠️ Manual intervention required"}`,
  )
  .join("\n")}`,
          },
        ],
