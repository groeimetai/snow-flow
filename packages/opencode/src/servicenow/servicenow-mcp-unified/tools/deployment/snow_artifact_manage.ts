    ...
    properties: {
      ...
      type: {
        ...
        enum: [
          ...
          'fix_script', 'table', 'field', 'flow', 'application'
        ]
      },
      sys_id: {
        type: 'string',
      ...
      internal_type: {
        ...
        enum: ['string', 'integer', 'boolean', 'reference', 'glide_date', 'glide_date_time',
               ...
               'email', 'phone_number_e164', 'currency', 'price']
      },
      reference_table: {
        type: 'string',
...
export async function execute(args: any, context: ServiceNowContext): Promise<ToolResult> {
  ...
  if (!tableName) {
    return createErrorResult(`Unsupported artifact type: ${type}. Valid types: ${Object.keys(ARTIFACT_TABLE_MAP).join(', ')}`);
  }
...
async function executeCreate(args: any, context: ServiceNowContext, tableName: string): Promise<ToolResult> {
  ...
  if (validate_es5) {
    if (server_script) {
      ...
      if (!validation.valid) {
        warnings.push(`Server script contains ES6+ syntax (${validation.violations.map((v: any) => v.type).join(', ')}). Consider using ES5 for compatibility.`);
      }
    }
    if (script) {
      const validation = validateES5Syntax(script);
      if (!validation.valid) {
        warnings.push(`Script contains ES6+ syntax (${validation.violations.map((v: any) => v.type).join(', ')}). Consider using ES5 for compatibility.`);
      }
    }
  }
...
async function executeDelete(args: any, context: ServiceNowContext, tableName: string): Promise<ToolResult> {
  ...
  if (!force) {
    ...
    if (dependencies.length > 0) {
      return createErrorResult(
        `Cannot delete ${type} with dependencies. Use force=true to override.`,
...
async function executeList(args: any, context: ServiceNowContext, tableName: string): Promise<ToolResult> {
  ...
  var response = await client.get(`/api/now/table/${tableName}`, {
    params: {
      ...
      sysparm_order: 'desc',
      ...(fields ? { sysparm_fields: fields } : { sysparm_fields: 'sys_id,name,id,title,short_description,active,sys_updated_on' })
    }
  });
...
async function executeAnalyze(args: any, context: ServiceNowContext, tableName: string): Promise<ToolResult> {
  ...
  if (scriptContent) {
    ...
    if (tableMatches) {
      var tables = [...new Set(tableMatches.map((m: string) => {
        var match = m.match(/['"]([^'"]+)['"]/);
    ...
    if (scriptIncludeMatches) {
      var scriptIncludes = [...new Set(scriptIncludeMatches.map((m: string) => {
  ...
  if (format === 'files') {
    ...
    for (var key of Object.keys(artifact)) {
      if (artifact[key] !== null && artifact[key] !== undefined) {
        var escaped = String(artifact[key])
          .replace(/&/g, '&amp;')
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;');
...
async function findArtifactByIdOrIdentifier(client: any, tableName: string, sysId?: string, identifier?: string): Promise<any> {
  // Try by sys_id first
  if (sysId) {
    try {
