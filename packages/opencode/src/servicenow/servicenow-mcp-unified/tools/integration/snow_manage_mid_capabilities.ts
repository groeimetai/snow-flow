  ...
  try {
    ...
    if (action === 'list') {
      ...
      if (use_case !== 'all') {
        ...
        capabilityDetails = capabilityDetails.filter(function(cap: any) {
          for (var j = 0; j < keywords.length; j++) {
            if ((cap.name && cap.name.indexOf(keywords[j]) >= 0) ||
      ...
      for (var k = 0; k < statusCaps.length; k++) {
        ...
        if (capNameLower.indexOf('discovery') >= 0 || capNameLower.indexOf('snmp') >= 0 ||
            capNameLower.indexOf('wmi') >= 0 || capNameLower.indexOf('ssh') >= 0) {
        ...
        } else if (capNameLower.indexOf('rest') >= 0 || capNameLower.indexOf('http') >= 0 ||
                   capNameLower.indexOf('jdbc') >= 0 || capNameLower.indexOf('soap') >= 0) {
      ...
      var recommendations: any = {
        discovery: [
          'SSH', 'WMI', 'SNMP', 'PowerShell', 'VMware',
          'Network Discovery', 'IP Network', 'Pattern Probe'
        ],
        orchestration: [
          'PowerShell', 'SSH', 'REST', 'Orchestration',
          'Script Execution', 'Remote Commands'
        ],
        integration: [
          'REST', 'HTTP', 'SOAP', 'JDBC', 'File Transfer',
          'Email', 'LDAP'
        ],
        service_mapping: [
          'Pattern Probe', 'Service Mapping', 'Discovery',
          'SSH', 'WMI', 'Network Discovery'
        ],
        cloud: [
