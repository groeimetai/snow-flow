  ...
  try {
    ...
    switch (action) {
      case 'list': {
        ...
        const response = await client.get('/api/now/table/sysevent_email_action', {
          params: {
            ...
            sysparm_fields: 'sys_id,name,collection,event_name,active,subject,condition,template,action_insert,action_update,type,recipient_fields,sys_created_on,sys_updated_on',
      ...
      case 'create': {
        ...
        // Recipients
        if (recipient_fields) createData.recipient_fields = Array.isArray(recipient_fields) ? recipient_fields.join(',') : recipient_fields;
        if (recipient_groups) createData.recipient_groups = Array.isArray(recipient_groups) ? recipient_groups.join(',') : recipient_groups;
        if (recipient_users) createData.recipient_users = Array.isArray(recipient_users) ? recipient_users.join(',') : recipient_users;
      ...
      case 'update': {
        ...
        // Recipients
        if (recipient_fields !== undefined) updateData.recipient_fields = Array.isArray(recipient_fields) ? recipient_fields.join(',') : recipient_fields;
        if (recipient_groups !== undefined) updateData.recipient_groups = Array.isArray(recipient_groups) ? recipient_groups.join(',') : recipient_groups;
        if (recipient_users !== undefined) updateData.recipient_users = Array.isArray(recipient_users) ? recipient_users.join(',') : recipient_users;
      ...
      case 'clone': {
        ...
        return createSuccessResult({
          ...
          message: 'Notification cloned successfully. The clone is ' + (cloneData.active ? 'active' : 'inactive') + ' by default.',
          url: context.instanceUrl + '/nav_to.do?uri=sysevent_email_action.do?sys_id=' + clonedSysId
        });
      }

      default:
        throw new SnowFlowError(ErrorType.VALIDATION_ERROR, 'Unknown action: ' + action + '. Valid actions: list, get, create, update, delete, enable, disable, test, clone');
    }
