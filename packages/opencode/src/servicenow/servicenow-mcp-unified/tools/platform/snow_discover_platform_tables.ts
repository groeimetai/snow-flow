...
async function queryCategory(
  ...
  limit: number,
  timeout: number
): Promise<{ category: string; description: string; count: number; tables: any[]; error?: string; timedOut?: boolean }> {
  try {
...
export async function execute(args: any, context: ServiceNowContext): Promise<ToolResult> {
  ...
  try {
    ...
    if (categoriesToQuery.length === 0) {
      return createErrorResult(`Unknown category: ${category}. Valid categories: all, ui, script, policy, action, security, system, itsm, cmdb`);
    }
    ...
    const queryPromises = categoriesToQuery.map(cat =>
      queryCategory(client, cat, limit, timeout)
    );
