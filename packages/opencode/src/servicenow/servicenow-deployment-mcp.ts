      ...
      tools: [
        {
          ...
          description:
            "Validates deployment artifacts for compatibility, dependencies, and permissions before execution. Returns validation report with potential issues.",
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          description:
            "Performs safe rollback of a failed deployment to previous state. Tracks rollback history and provides recovery recommendations.",
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          description:
            "Exports ServiceNow artifacts (widgets, applications) to JSON/XML format for backup, version control, or migration purposes.", // removed workflows
          inputSchema: {
            type: "object",
            properties: {
              type: { type: "string", enum: ["widget", "application"] }, // removed 'workflow' - deprecated
        ...
        },
        {
          ...
          description:
            "Validates widget coherence by analyzing server/client/HTML component communication. Checks data bindings, action handlers, and method implementations.",
          inputSchema: {
            type: "object",
            properties: {
              ...
              validation_mode: {
                ...
                description:
                  "Validation mode: full (all components), template_only (HTML bindings), data_only (server data)",
                default: "full",
              },
            },
          },
        ...
        },
        {
          name: "snow_update",
          description:
            "Updates existing ServiceNow artifacts (widgets, applications, etc.). Finds artifact by name or sys_id and modifies it. Use snow_create_artifact for creating new artifacts.",
          inputSchema: {
            type: "object",
            properties: {
              type: {
      ...
      if (hasServicePortalAccess) {
        try {
          ...
          if (verificationResult.exists && verificationResult.data) {
            ...
            throw new Error(
              `Widget creation failed: ${result?.error || "Unknown error"}${errorDetails ? "\nDetails: " + errorDetails : ""}`,
            )
          }
        } catch (error: any) {
          directError = error
          ...
          try {
            ...
            if (verificationResult.exists && verificationResult.data) {
              ...
              const createdArtifact = verificationResult.data
              this.logger.info("✅ Widget found! Deployment succeeded despite error", {
                sys_id: createdArtifact.sys_id,
              })
      ...
      if (result && result.success && result.data) {
        ...
        try {
          ...
          if (!isValid) {
            ...
            this.logger.warn(
              `Widget deployed successfully but immediate validation check failed - this is normal for new records`,
            )
...
📚 Documentation: See CLAUDE.md for Widget Deployment Guidelines`
      ...
      const is403Error =
        error?.response?.status === 403 || error?.message?.includes("403") || error?.message?.includes("Forbidden")
      ...
      if (is403Error) {
        ...
        try {
          ...
          const apiResponse = await this.client.get(
            `/api/now/table/sp_widget?sysparm_query=name=${encodeURIComponent(args.name)}&sysparm_limit=1&sysparm_fields=sys_id,name,title`,
          )
  ...
  private async deployPortalPage(args: any) {
    try {
      ...
      if (!args.page_id || !args.title) {
        ...
        throw new Error(
          `Portal page must have page_id and title. Received: page_id=${args.page_id}, title=${args.title}`,
        )
      }
      ...
      if (widgetsToCreate.length > 0) {
        for (const widgetConfig of widgetsToCreate) {
          ...
          if (widgetConfig.widget && widgetConfig.widget !== args.widget_name) {
            try {
              const widgetLookup = await this.client.searchRecords(
                "sp_widget",
                `sys_id=${widgetConfig.widget}^ORname=${widgetConfig.widget}`,
                1,
              )
...
${widgetInstances
  .map((instance, i) => `${i + 1}. Widget: ${instance.widget} (Instance: ${instance.sys_id})`)
  .join("\n")}
`
...
${
  args.widgets && args.widgets.length > 0
    ? args.widgets
        .map(
          (w: any, i: number) => `
  Widget ${i + 1}:
...
📚 Documentation: See CLAUDE.md for Flow Development Guidelines`;
  ...
  private async deployBusinessRule(artifact: any): Promise<any> {
    ...
    if (!result.data?.sys_id) {
      throw new Error(
        `Business Rule deployment failed: No sys_id returned from ServiceNow. Result: ${JSON.stringify(result)}`,
      )
    }
...
💡 **Scope Benefits:**
${this.getScopeBenefits(deploymentResult.scope)
  .map((b) => `- ${b}`)
  .join("\n")}`,
          },
        ],
    ...
    try {
      ...
      if (args.type === "widget") {
        ...
        try {
          ...
          validationResults.push(
            `⚠️ API validation limited: ${apiError instanceof Error ? apiError.message : String(apiError)}`,
          )
        }
        ...
        if (args.artifact.name) {
          try {
            ...
            validationResults.push(
              `⚠️ Workflow validation limited: ${workflowError instanceof Error ? workflowError.message : String(workflowError)}`,
            )
          }
        } else {
          hasErrors = true
        ...
        if (args.artifact.name) {
          try {
            ...
            validationResults.push(
              `⚠️ Application validation limited: ${appError instanceof Error ? appError.message : String(appError)}`,
            )
          }
        } else {
          hasErrors = true
          ...
          content: [
            {
              type: "text",
              text:
                `❌ TEST-002 FIX: Missing update_set_id parameter\n\n` +
            ...
            content: [
              {
                type: "text",
                text:
                  `❌ TEST-002 FIX: Update Set not found (404 error)\n\n` +
          ...
          content: [
            {
              type: "text",
              text:
                `⚠️ Cannot rollback Update Set: ${updateSet.name}\n\n` +
                `📋 **Update Set Details:**\n` +
                `   - Name: ${updateSet.name}\n` +
        ...
        content: [
          {
            type: "text",
            text:
              `❌ TEST-002 FIX: Rollback system error\n\n` +
              `📝 **Error:** ${error instanceof Error ? error.message : String(error)}\n\n` +
              `🛠️ **Recovery options:**\n` +
...
<unload unload_date="${this.escapeXMLText(timestamp)}">
  ...
  private async createUpdateSetExport(
    artifacts: Array<{ table: string; sys_id: string; data: any }>,
    name: string,
  ): Promise<string> {
    const timestamp = new Date().toISOString()
    ...
    try {
      ...
      try {
        ...
        const systemInfoResponse = await this.client.get("/api/now/table/sys_properties", {
          sysparm_query:
            "name=glide.servlet.uri^ORname=instance.name^ORname=glide.db.rdbms^ORname=glide.war.production",
...
${
  serviceNowInfo.tableAccess
    ? serviceNowInfo.tableAccess
        .map((test: any) => `- ${test.name}: ${test.status}${test.error ? ` (${test.error})` : ""}`)
        .join("\n")
...
**🔍 Tracked Artifacts:**
${
  sessionSummary.artifacts.length > 0
    ? sessionSummary.artifacts
        .map(
          (a) =>
            `- ${a.name} (${a.type})
    Sys ID: ${a.sys_id}
      ...
      try {
        ...
        try {
          ...
          realApiTests.updateSetAccess = {
            ...
            details:
              updateSetTest?.result?.length > 0
                ? `Found ${updateSetTest.result.length} update sets`
...
- admin (system access if needed)`,
          ...
          if (entries.length > 0) {
            testResults = entries
              .map(([name, result]: [string, any]) => {
                ...
                const httpStatus =
                  result?.http_status && typeof result.http_status === "number"
  ...
  ${httpStatus}`
      ...
      const urlFixRecommendation =
        data.instance_url && typeof data.instance_url === "string" && data.instance_url.includes("//")
...
<record_update table="sys_hub_flow">
   ...
   */
  private generateTriggerInstanceXML(
    args: any,
    flowId: string,
    triggerId: string,
    timestamp: string,
...
<record_update table="sys_hub_trigger_instance">
   ...
   */
  private generateActionInstanceXML(
    activity: any,
    actionId: string,
    flowId: string,
    sequence: number,
...
Use \`snow_validate_widget_coherence\` to validate the widget component communication.`,
  ...
  private analyzeCodeCoverage(widget: any): any {
    ...
    const templateVars = (widget.template?.match(/\{\{([^}]+)\}\}/g) || []).map((v: string) =>
      v.replace(/[{}]/g, "").trim(),
    )
    const clientBindings = (widget.client_script?.match(/c\.(\w+)|\$scope\.(\w+)/g) || []).map((v: string) =>
      v.replace(/c\.|\\$scope\./, ""),
    )
    const serverDataKeys = (widget.script?.match(/data\.(\w+)/g) || []).map((v: string) => v.replace("data.", ""))
    ...
    coverage.templateCoverage =
      coverage.totalTemplateVars > 0 ? Math.round((coverage.templateVarsUsed / coverage.totalTemplateVars) * 100) : 100
    coverage.clientCoverage =
      coverage.totalClientBindings > 0
        ? Math.round((coverage.clientBindingsUsed / coverage.totalClientBindings) * 100)
        : 100
    coverage.serverCoverage =
      coverage.totalServerData > 0 ? Math.round((coverage.serverDataUsed / coverage.totalServerData) * 100) : 100
  ...
  private checkIntegration(integration: any): string {
    ...
    const undefinedVars = integration.template_refs.filter((ref: string) => {
      ...
      return (
        !integration.client_bindings.some((binding: string) => binding.includes(varName)) &&
      ...
      for (const artifact of args.artifacts) {
        try {
          ...
          switch (artifact.type) {
            case "flow":
              throw new Error(
                "Flow deployment is deprecated. Flows, workflows, and subflows are no longer supported. Use widgets or applications instead.",
              )
              break
            case "widget":
  ...
  private async unifiedDeploy(args: any) {
    try {
      ...
      const {
        ...
        permission_escalation = "auto_request",
        deployment_context,
      } = args
...
Use \`snow_create_artifact\` with type: '${args.type}' and manual configuration.`,
  ...
  private async processNaturalLanguageInstruction(type: string, instruction: string): Promise<any> {
    ...
    if (type === "widget") {
      ...
      const widgetTitle =
        widgetName.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()) || "Auto Generated Widget"
  ...
  private extractNameFromInstruction(instruction: string): string {
    ...
    const name = words
      .filter(
        (word) =>
          word.length > 2 &&
          !["the", "and", "for", "with", "that", "will", "can", "should", "create", "make", "build", "new"].includes(
            word,
          ),
      )
      .slice(0, 3)
  ...
  private generateWidgetTemplate(instruction: string): string {
    const lower = instruction.toLowerCase()
    const title = this.extractNameFromInstruction(instruction)
      .replace(/_/g, " ")
    ...
    try {
      ...
      if (auto_preview) {
        ...
        if (previewProblems.result && previewProblems.result.length > 0) {
          const problems = previewProblems.result.map((p: any) => `- ${p.type}: ${p.description}`).join("\n")
      ...
      for (let i = 0; i < actions.length; i++) {
        ...
        if (action.catalog_variable && !action.catalog_variable.match(/^[a-f0-9]{32}$/)) {
          ...
          if (actionVariableSearch.success && actionVariableSearch.data.result.length > 0) {
            ...
            this.logger.info(
              `✅ Resolved action variable '${action.catalog_variable}' to sys_id: ${actionVariableSysId}`,
            )
          } else {
        ...
        if (action.type === "set_visible" || action.visible !== undefined) {
          actionData.visible = action.visible === true ? "true" : action.visible === false ? "false" : "ignore"
        } else {
...
**Error Details**: ${error.message || error}`
   ...
   */
  private async universalArtifactVerification(
    artifactType: string,
    identifier: string | { sys_id?: string; name?: string },
    table?: string,
  ): Promise<any> {
   ...
   */
  private async universalDirectApiVerification(
    artifactType: string,
    identifier: string,
    ...
    try {
      ...
      const apiResponse = await this.client.get(
        `/api/now/table/${targetTable}?sysparm_query=name=${encodeURIComponent(identifier)}&sysparm_limit=1&sysparm_fields=sys_id,name,title`,
      )
  ...
  private async checkArtifactExists(
    artifactType: string,
    identifier: string,
  ): Promise<{ exists: boolean; artifact?: any }> {
    try {
...
💡 **Success**: Unified deployment workflow completed successfully!`,
   ...
   */
  private async generateManualDeploymentSteps(
    type: string,
    config: any,
    error: any,
    updateSetSession: any,
  ): Promise<any> {
    const credentials = await this.oauth.loadCredentials()
...
${
  updateSetSession
    ? `📋 **Update Set Ready**: ${updateSetSession.name} (${updateSetSession.update_set_id})
✅ Manual changes will be automatically tracked in this Update Set.
...
${
  updateSetSession
    ? `📋 **Update Set**: ${updateSetSession.name}
🆔 **Session ID**: ${updateSetSession.update_set_id}`
    : "⚠️ **No Update Set Session** - Create one manually before making changes"
}
...
💡 **Benefit**: Even though automatic deployment failed, you have a complete deployment package ready.`,
   ...
   */
  private async trackArtifactInUpdateSet(
    updateSetId: string,
    artifact: { type: string; sys_id: string; name: string },
  ): Promise<void> {
    try {
...
- Find by name: \`snow_update({ type: "${type}", identifier: "exact_name" })\``,
      ...
      if (updateSetSession) {
        artifactTracker.trackArtifact(existingArtifact.sys_id, tableName, existingArtifact.name, type, "update")
      }
...
- Ready for promotion to higher environments`
    : "🔄 Changes made outside Update Set - track manually"
}
    ...
    try {
      ...
      switch (type) {
        ...
        case "script_include":
          ...
          break
        case "ui_action":
          await this.parseUIActionInstruction(
            updateData,
            lowerInstruction,
            codeBlocks,
            explicitValues,
          ...
          break
        case "ui_policy":
          await this.parseUIPolicyInstruction(
            updateData,
            lowerInstruction,
            codeBlocks,
            explicitValues,
          ...
          break
        case "notification":
          await this.parseNotificationInstruction(
            updateData,
            lowerInstruction,
            codeBlocks,
            explicitValues,
        ...
        case "field":
          ...
          break
        case "workflow":
          await this.parseWorkflowInstruction(
            updateData,
            lowerInstruction,
            codeBlocks,
            explicitValues,
        ...
        case "flow":
          ...
          break
        case "scheduled_job":
          await this.parseScheduledJobInstruction(
            updateData,
            lowerInstruction,
            codeBlocks,
            explicitValues,
...
snow_update({ 
  ...
  config: { 
    field_name: "new_value"
      ...
      this.logger.error("Failed to process update instruction", {
   ...
   */
  private async parseWidgetInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
...
c.$onInit = function() {
  ...
  c.initChart();
    ...
    if (instruction.includes("change") && instruction.includes("title")) {
      const titleMatch =
        instruction.match(/title[:\s]+["']([^"']+)["']/i) || instruction.match(/to\s+["']([^"']+)["']/i)
   ...
   */
  private async parseScriptInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseUIActionInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseUIPolicyInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseNotificationInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseACLInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseTableInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseFieldInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseWorkflowInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseFlowInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   */
  private async parseScheduledJobInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
   ...
   * Generic parsing for unsupported types
   */
  private async parseGenericInstruction(
    updateData: any,
    instruction: string,
    codeBlocks: any,
    explicitValues: any,
