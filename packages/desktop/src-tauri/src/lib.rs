...
fn get_sidecar_port() -> u32 {
    option_env!("SNOW_CODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("SNOW_CODE_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, hostname: &str, port: u32, password: &str) -> CommandChild {
    ...
    .env("SNOW_CODE_SERVER_USERNAME", "opencode")
    .env("SNOW_CODE_SERVER_PASSWORD", password)
    .spawn()
        ...
        .setup(move |app| {
            ...
            let window_builder = WebviewWindowBuilder::from_config(&app, config)
                ...
                .initialization_script(format!(
                    r#"
                      window.__SNOW_CODE__ ??= {{}};
                      window.__SNOW_CODE__.updaterEnabled = {updater_enabled};
                    "#
                ));
