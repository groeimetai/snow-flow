...
try {
  ...
  console.log("snow-code session", session.id)
  if (shareId) {
...
function createOpencode() {
  ...
  const proc = spawn(`snow-code`, [`serve`, `--hostname=${host}`, `--port=${port}`])
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/snow-code|\/sc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/snow-code` or `/sc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to snow-code server")
  }
}
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/snow-code" || body === "/sc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/snow-code") || body.includes("/sc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to snow-code...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "snow-code-agent[bot]"`
  await $`git config --global user.email "snow-code-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `snow-code/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/snow-code-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[snow-code session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
