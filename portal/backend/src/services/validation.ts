...
export class ValidationService {
  ...
  async validate(
    ...
    try {
      ...
      const license = await this.db.getLicense(request.key);
      if (!license) {
        await this.db.logValidation(0, request.instanceId, request.version, false, 'LICENSE_NOT_FOUND', ipAddress);
      ...
      if (!this.verifySignature(request, license.key)) {
        await this.db.logValidation(license.id, request.instanceId, request.version, false, 'INVALID_SIGNATURE', ipAddress);
        return this.errorResponse('INVALID_SIGNATURE', 'Invalid request signature');
      }

      // Step 5: Check license status
      if (license.status !== 'active') {
        await this.db.logValidation(license.id, request.instanceId, request.version, false, `LICENSE_${license.status.toUpperCase()}`, ipAddress);
      ...
      if (license.expiresAt && license.expiresAt < now) {
        // Auto-expire license
        await this.db.updateLicense(license.key, { status: 'expired' });
        await this.db.logValidation(license.id, request.instanceId, request.version, false, 'LICENSE_EXPIRED', ipAddress);
        return this.errorResponse('LICENSE_EXPIRED', 'License has expired');
      }

      // Step 7: Check instance limit
      const instanceCount = await this.db.getInstanceCount(license.id);
      const warnings: string[] = [];

      // If this is a new instance and we're at the limit
      const existingInstance = await this.db.upsertInstance(
        license.id,
        request.instanceId,
        request.version,
        ipAddress,
      ...
      if (instanceCount >= license.maxInstances && existingInstance.createdAt === existingInstance.lastSeen) {
        ...
        await this.db.logValidation(license.id, request.instanceId, request.version, false, 'INSTANCE_LIMIT_EXCEEDED', ipAddress);
      ...
      await this.db.logValidation(license.id, request.instanceId, request.version, true, undefined, ipAddress);
  ...
  async getStats(licenseKey: string): Promise<{
    ...
    const instances = await this.db.getInstanceCount(license.id);
    const validations = await this.db.getValidationStats(license.id, 30);
