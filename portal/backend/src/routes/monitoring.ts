...
export function createMonitoringRoutes(db: LicenseDatabase, credsDb: CredentialsDatabase): Router {
  ...
  router.get('/health/detailed', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const customerPromises = serviceIntegrators.map(si => db.listCustomers(si.id));
    ...
    try {
      ...
      const serviceIntegrators = await db.listServiceIntegrators();
      let totalCreds = 0;
      for (const si of serviceIntegrators) {
        const customers = await db.listCustomers(si.id);
        for (const c of customers) {
          const creds = await credsDb.listCustomerCredentials(c.id);
