...
brew install serac-labs/tap/serac # macOS 與 Linux（推薦，始終保持最新）
brew install serac-labs/tap/serac              # macOS 與 Linux（官方 brew formula，更新頻率較低）
paru -S serac-bin               # Arch Linux
mise use -g serac               # 任何作業系統
nix run nixpkgs#serac           # 或使用 github:serac-labs/serac 以取得最新開發分支
```
...
- 100% 開源。
- 不綁定特定的服務提供商。雖然我們推薦使用透過 [Serac Zen](https://serac.build/zen) 提供的模型，但 Serac 也可搭配 Claude, OpenAI, Google 甚至本地模型使用。隨著模型不斷演進，彼此間的差距會縮小且價格會下降，因此具備「不限廠商 (provider-agnostic)」的特性至關重要。
- 內建 LSP (語言伺服器協定) 支援。
- 專注於終端機介面 (TUI)。Serac 由 Neovim 愛好者與 [terminal.shop](https://terminal.shop) 的創作者打造；我們將不斷挑戰終端機介面的極限。
- 客戶端/伺服器架構 (Client/Server Architecture)。這讓 Serac 能夠在您的電腦上運行的同時，由行動裝置進行遠端操控。這意味著 TUI 前端只是眾多可能的客戶端之一。
