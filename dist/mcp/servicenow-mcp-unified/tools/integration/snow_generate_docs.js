...
async function execute(args, context) {
    const { artifact_type, sys_id, include_dependencies = true, include_usage_examples = true, format = 'markdown' } = args;
    try {
...
function generateUsageExamples(artifactType, artifact) {
    const examples = [];
    switch (artifactType) {
        case 'widget':
...
function formatMarkdown(doc) {
    ...
    if (doc.dependencies && doc.dependencies.length > 0) {
        md += `## Dependencies\n`;
        doc.dependencies.forEach((dep) => md += `- ${dep}\n`);
        md += '\n';
    }
    if (doc.usage_examples && doc.usage_examples.length > 0) {
        md += `## Usage Examples\n\n`;
        doc.usage_examples.forEach((example, index) => {
            md += `### Example ${index + 1}\n\`\`\`javascript\n${example}\n\`\`\`\n\n`;
        });
    }
    return md;
...
function formatHTML(doc) {
    ...
    if (doc.dependencies && doc.dependencies.length > 0) {
        ...
        doc.dependencies.forEach((dep) => html += `<li>${dep}</li>`);
        html += `</ul>`;
    }
    html += `</body></html>`;
    return html;
...
//# sourceMappingURL=snow_generate_docs.js.map