    ...
    try {
        ...
        if (include_performance) {
            const durations = executions
                ...
                .map((exec) => parseInt(exec.duration));
            if (durations.length > 0) {
                ...
                const slowExecutions = executions.filter((exec) => parseInt(exec.duration) > slowThreshold);
        ...
        if (include_errors) {
            ...
            const errorExecutions = executions.filter((exec) => errorStates.includes(exec.state));
            ...
            if (errorExecutions.length > 0) {
                ...
                analysis.error_analysis.sample_errors = errorSample.map((exec) => ({
                    sys_id: exec.sys_id,
                    state: exec.state,
...
//# sourceMappingURL=snow_workflow_analyze.js.map