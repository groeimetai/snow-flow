...
async function execute(args, context) {
    const { query, limit = 10, include_content = false, include_analysis = false, fields } = args;
    try {
        ...
        if (include_content || (fields && fields.length > 0)) {
            ...
            if (incidents.length > 0) {
                result.summary.sample = {
                    ...
                    categories: [...new Set(incidents.slice(0, 5).map((inc) => inc.category || 'none'))],
                    priorities: [...new Set(incidents.slice(0, 5).map((inc) => inc.priority || 'none'))],
                    states: [...new Set(incidents.slice(0, 5).map((inc) => inc.state || 'unknown'))]
                };
            }
        }
        ...
        if (include_analysis && incidents.length > 0) {
            ...
            incidents.forEach((incident) => {
                ...
                if (description.includes('network'))
                    patterns.set('network_issues', (patterns.get('network_issues') || 0) + 1);
                if (description.includes('database'))
                    patterns.set('database_issues', (patterns.get('database_issues') || 0) + 1);
                if (description.includes('password'))
                    patterns.set('auth_issues', (patterns.get('auth_issues') || 0) + 1);
            });
            result.analysis = {
        ...
        return (0, error_handler_js_1.createSuccessResult)(result, { query: processedQuery, limit, count: incidents.length });
    }
    catch (error) {
...
//# sourceMappingURL=snow_query_incidents.js.map