...
function processNaturalLanguageQuery(query) {
    ...
    if (lowerQuery.includes('open') || lowerQuery.includes('active'))
        return 'active=true^request_state!=closed';
    if (lowerQuery.includes('closed') || lowerQuery.includes('complete'))
        return 'request_state=closed';
    if (lowerQuery.includes('pending approval'))
...
async function execute(args, context) {
    ...
    try {
            ...
            for (const request of requests) {
                ...
                if (itemResponse.data.result && itemResponse.data.result.length > 0) {
                    requestItems.push({
                        ...
                        items: itemResponse.data.result.map((item) => ({
                            number: item.number,
        ...
        return (0, error_handler_js_1.createSuccessResult)(result, { query: processedQuery, limit, count: requests.length });
    }
    catch (error) {
...
//# sourceMappingURL=snow_query_requests.js.map