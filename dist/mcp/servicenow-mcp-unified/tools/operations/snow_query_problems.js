...
async function execute(args, context) {
    ...
    try {
            ...
            for (const problem of problems) {
                ...
                if (incidentResponse.data.result && incidentResponse.data.result.length > 0) {
                    relatedIncidents.push({
                        ...
                        incidents: incidentResponse.data.result.map((inc) => ({
                            number: inc.number,
        ...
        return (0, error_handler_js_1.createSuccessResult)(result, { query: processedQuery, limit, count: problems.length });
    }
    catch (error) {
...
//# sourceMappingURL=snow_query_problems.js.map