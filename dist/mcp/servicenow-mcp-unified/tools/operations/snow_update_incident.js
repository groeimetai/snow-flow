...
async function execute(args, context) {
    const { sys_id, state, assigned_to, assignment_group, work_notes, comments, resolution_code, resolution_notes, close_code, close_notes, validate_state_transition = true } = args;
    try {
        ...
        if (!incident) {
            throw new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.NOT_FOUND_ERROR, `Incident not found: ${sys_id}`, { details: { sys_id } });
        }
        const updateData = {};
        ...
        if (state && validate_state_transition) {
            ...
            if (!validTransitions[currentState]?.includes(state)) {
                throw new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.VALIDATION_ERROR, `Invalid state transition from ${currentState} to ${state}`, {
                    details: {
            ...
            if (state === 6) { // Resolved
                if (!resolution_code || !resolution_notes) {
                    throw new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.VALIDATION_ERROR, 'Resolution code and notes are required when resolving', { details: { missing_fields: ['resolution_code', 'resolution_notes'] } });
                }
            ...
            if (state === 7) { // Closed
                if (!close_code || !close_notes) {
                    throw new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.VALIDATION_ERROR, 'Close code and notes are required when closing', { details: { missing_fields: ['close_code', 'close_notes'] } });
                }
...
//# sourceMappingURL=snow_update_incident.js.map