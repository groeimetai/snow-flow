...
async function execute(args, context) {
    const { table = 'task', sys_id, assigned_to, assignment_group, auto_assign = false, check_availability = false, work_notes } = args;
    try {
        ...
        if (!task) {
            throw new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.NOT_FOUND_ERROR, `Task not found: ${sys_id}`, { details: { table, sys_id } });
        }
        const updateData = {};
        ...
        if (assigned_to) {
            ...
            if (assigned_to.length === 32) {
                ...
                if (userResponse.data.result && userResponse.data.result.length > 0) {
                    ...
                    if (!user.active) {
                        throw new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.VALIDATION_ERROR, `User '${assigned_to}' is inactive and cannot be assigned tasks`, { details: { user: assigned_to } });
                    }
            ...
            if (groupMembers.data.result && groupMembers.data.result.length > 0) {
                const memberIds = groupMembers.data.result.map((m) => m.user.value);
                // Get workload for each member
                const workloads = await Promise.all(memberIds.map(async (userId) => {
...
//# sourceMappingURL=snow_assign_task.js.map