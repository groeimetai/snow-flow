...
async function execute(args, context) {
    ...
    try {
        ...
        if (data.length === 0) {
            return (0, error_handler_js_1.createErrorResult)('No data found in table');
        }
        ...
        if (analysis.completeness)
            scores.push(analysis.completeness.score);
        if (analysis.consistency)
            scores.push(analysis.consistency.score);
        if (analysis.accuracy)
            scores.push(analysis.accuracy.score);
...
function analyzeCompleteness(data, fields) {
...
function analyzeConsistency(data, fields) {
    const fieldsToCheck = fields || Object.keys(data[0] || {});
    const total = fieldsToCheck.length;
    let consistent = 0;
    fieldsToCheck.forEach(field => {
...
function analyzeAccuracy(data, fields) {
    const fieldsToCheck = fields || Object.keys(data[0] || {});
    const total = fieldsToCheck.length;
    let accurate = 0;
    fieldsToCheck.forEach(field => {
...
//# sourceMappingURL=snow_analyze_data_quality.js.map