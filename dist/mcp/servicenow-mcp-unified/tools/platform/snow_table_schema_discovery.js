...
async function execute(args, context) {
    const { table_name, include_fields = true, include_relationships = true, include_acls = false, include_business_rules = false, include_ui_policies = false } = args;
    try {
        ...
        if (!tableResponse.data.result || tableResponse.data.result.length === 0) {
            return (0, error_handler_js_1.createErrorResult)(`Table '${table_name}' not found`);
        }
        ...
        return (0, error_handler_js_1.createSuccessResult)({ schema }, { table_name });
    }
    catch (error) {
        return (0, error_handler_js_1.createErrorResult)(error.message);
    }
}
async function discoverFields(client, tableName) {
...
async function discoverRelationships(client, tableName) {
    const relationships = {
        references_from: [],
        references_to: [],
        extensions: []
    ...
    relationships.references_from = referencesFrom.map((ref) => ({
        table: ref.name,
        field: ref.element,
    ...
    relationships.references_to = referencesTo.map((ref) => ({
        field: ref.element,
    ...
    relationships.extensions = extensions.map((ext) => ({
        table: ext.name,
        label: ext.label
    }));
    return relationships;
}
async function discoverACLs(client, tableName) {
...
async function discoverBusinessRules(client, tableName) {
...
async function discoverUIPolicies(client, tableName) {
...
//# sourceMappingURL=snow_table_schema_discovery.js.map