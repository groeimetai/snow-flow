...
async function execute(args, context) {
    ...
    try {
        ...
        if (conversation_id) {
            ...
            return (0, error_handler_js_1.createErrorResult)('Either conversation_id or user_id must be provided');
        }
        ...
        if (!convResponse.data.result || convResponse.data.result.length === 0) {
            return (0, error_handler_js_1.createErrorResult)('Conversation not found');
        }
        const conversation = convResponse.data.result[0];
        let transcript = [];
        if (include_transcript) {
            const messagesResponse = await client.get(`/api/now/table/sys_cs_message?sysparm_query=conversation=${conversation.sys_id}&sysparm_limit=${limit}&sysparm_orderby=sys_created_on`);
            if (messagesResponse.data.result) {
                transcript = messagesResponse.data.result.map((m) => ({
                    author: m.author,
                    text: m.text,
...
//# sourceMappingURL=snow_get_va_conversation.js.map