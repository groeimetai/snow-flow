    ...
    try {
        ...
        if (coverage) {
            ...
            if (coverageReport.data_bindings.length > 0) {
                const coveredBindings = coverageReport.data_bindings.filter((binding) => coverageReport.server_properties.includes(binding)).length;
        ...
        const message = `🧪 Widget Test Results\n\n` +
            ...
            `Scenarios:\n${testResults.scenarios.map((s) => `${s.passed ? '✅' : '❌'} ${s.name}${s.errors.length > 0 ? `: ${s.errors.join(', ')}` : ''}`).join('\n')}`;
        return (0, error_handler_js_1.createSuccessResult)(testResults, { message });
    }
    catch (error) {
        return (0, error_handler_js_1.createErrorResult)(error instanceof error_handler_js_1.SnowFlowError
            ? error
            : new error_handler_js_1.SnowFlowError(error_handler_js_1.ErrorType.NETWORK_ERROR, `Widget test failed: ${error.message}`, { originalError: error }));
    }
}
...
//# sourceMappingURL=snow_widget_test.js.map