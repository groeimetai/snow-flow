...
async function execute(args, context) {
    const { cat_item, short_description, condition, applies_to = 'item', active = true, on_load = true, reverse_if_false = true, conditions, actions } = args;
    try {
        ...
        if (condition)
            policyData.catalog_conditions = condition;
        ...
        if (actions && Array.isArray(actions)) {
            for (const action of actions) {
                ...
                if (action.mandatory !== undefined)
                    actionData.mandatory = action.mandatory;
                if (action.visible !== undefined)
                    actionData.visible = action.visible;
                if (action.readonly !== undefined)
                    actionData.disabled = action.readonly;
...
//# sourceMappingURL=snow_create_catalog_ui_policy.js.map