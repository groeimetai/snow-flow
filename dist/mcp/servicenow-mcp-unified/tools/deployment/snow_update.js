...
exports.toolDefinition = {
    ...
    use_cases: ['deployment', 'update', 'artifacts'],
...
async function execute(args, context) {
    ...
    try {
        ...
        if (!tableName) {
            return (0, error_handler_js_1.createErrorResult)(`Unsupported artifact type: ${type}`);
        }
        // Find the artifact
        const artifact = await findArtifact(client, tableName, identifier);
        if (!artifact) {
            return (0, error_handler_js_1.createErrorResult)(`${type} '${identifier}' not found`);
        }
        ...
        if (validate) {
            ...
            if (!validation.valid) {
                return (0, error_handler_js_1.createErrorResult)(`Validation failed: ${validation.errors.join(', ')}`);
            }
        }
        ...
        return (0, error_handler_js_1.createSuccessResult)(result, { type, identifier });
    }
    catch (error) {
        return (0, error_handler_js_1.createErrorResult)(error.message);
    }
}
async function findArtifact(client, tableName, identifier) {
    // Try by sys_id first
    try {
...
async function validateUpdate(type, currentArtifact, updates) {
    const validation = {
        valid: true,
        errors: [],
        warnings: []
...
async function createArtifactBackup(client, tableName, sysId, artifact) {
    const backupData = {
        table: tableName,
        record_sys_id: sysId,
...
//# sourceMappingURL=snow_update.js.map