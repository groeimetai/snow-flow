...
program
    ...
    if (options.verbose) {
        ...
        if (options.autoDeploy) {
            console.log(chalk_1.default.green(`✅ ServiceNow integration active - will create real artifacts`));
        }
    ...
    if (options.verbose) {
        ...
        console.log(chalk_1.default.yellow('⚠️  Not authenticated - run ') + chalk_1.default.cyan('snow-flow auth login') + chalk_1.default.yellow(' first'));
    }
...
async function executeOpenCode(objective) {
    ...
    try {
        ...
        const hasEnvFile = (0, fs_2.existsSync)(envPath);
        if (!hasEnvFile) {
        ...
        console.log(chalk_1.default.green('✅ 235+ ServiceNow tools available via MCP'));
...
program
    ...
    const monitoringInterval = setInterval(() => {
        ...
        cliLogger.info('┌─────────────────────────────────────────────────────────────┐');
        console.log(`│               🚀 Snow-Flow Monitor v${version_js_1.VERSION}                   │`);
        console.log('├─────────────────────────────────────────────────────────────┤');
...
program
    ...
    .action(async (options) => {
    console.log(chalk_1.default.blue.bold(`\n🏔️ Snow-Flow v${version_js_1.VERSION} - Conversational ServiceNow Development`));
    console.log('='.repeat(60));
    const targetDir = process.cwd();
    try {
        // Check for .snow-flow migration
        const { migrationUtil } = await Promise.resolve().then(() => __importStar(require('./utils/migrate-snow-flow.js')));
        ...
        console.log(chalk_1.default.blue.bold('\n🎯 Next steps:'));
        console.log(chalk_1.default.red.bold('⚠️  IMPORTANT: Configure OpenCode first!'));
        console.log('1. Install OpenCode: ' + chalk_1.default.cyan('npm install -g @opencode/cli'));
        console.log('2. Authenticate Snow-Flow: ' + chalk_1.default.cyan('snow-flow auth login'));
        console.log('3. Import OpenCode config: ' + chalk_1.default.cyan('opencode config import opencode-config.example.json'));
        console.log('4. Start developing: ' + chalk_1.default.cyan('snow-flow swarm "create incident dashboard"'));
...
program
    ...
    .action(() => {
    console.log(`
🚀 Snow-Flow v${version_js_1.VERSION} - ServiceNow Multi-Agent Development Framework
...
program
    ...
    console.log(chalk_1.default.blue('ℹ️  MCP servers are now managed by Claude Agent SDK v0.1.1'));
    console.log(chalk_1.default.yellow('⚠️  Manual MCP commands are no longer needed'));
    console.log(chalk_1.default.green('✅ SDK automatically handles all MCP server lifecycle'));
...
async function handleMCPDebug(options) {
