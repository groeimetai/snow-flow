...
brew install serac-labs/tap/serac # macOS 和 Linux（推荐，始终保持最新）
brew install serac-labs/tap/serac              # macOS 和 Linux（官方 brew formula，更新频率较低）
paru -S serac-bin               # Arch Linux
mise use -g serac               # 任意系统
nix run nixpkgs#serac           # 或用 github:serac-labs/serac 获取最新 dev 分支
```
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [Serac Zen](https://serac.build/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。Serac 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
