...
Snow-Flow 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://snow-flow.dev/docs/agents) 에 대해 더 알아보세요.

### 문서

Snow-Flow 설정에 대한 자세한 내용은 [**문서**](https://snow-flow.dev/docs) 를 참고하세요.

### 기여하기

Snow-Flow 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.
...
- 100% 오픈 소스
- 특정 제공자에 묶여 있지 않습니다. [Snow-Flow Zen](https://snow-flow.dev/zen) 을 통해 제공하는 모델을 권장하지만, Snow-Flow 는 Claude, OpenAI, Google 또는 로컬 모델과도 사용할 수 있습니다. 모델이 발전하면서 격차는 줄고 가격은 내려가므로 provider-agnostic 인 것이 중요합니다.
- 기본으로 제공되는 LSP 지원
- TUI 에 집중. Snow-Flow 는 neovim 사용자와 [terminal.shop](https://terminal.shop) 제작자가 만들었으며, 터미널에서 가능한 것의 한계를 밀어붙입니다.
