...
export function createSsoRoutes(db: LicenseDatabase): express.Router {
   ...
   */
  function getSamlStrategy(customerId: number): SamlStrategy | null {
    // Check cache first
  ...
  router.get('/login/:customerId', ssoLoginRateLimiter, csrfProtection, (req: Request, res: Response) => {
    try {
      ...
      // Get SAML strategy
      const strategy = getSamlStrategy(customerId);
      if (!strategy) {
  ...
  router.post('/logout', async (req: Request, res: Response) => {
    try {
      ...
      if (session) {
        ...
        const ssoConfig = db.getSsoConfig(session.customerId);
   ...
   */
  router.get('/metadata/:customerId', (req: Request, res: Response) => {
    try {
      const customerId = parseInt(req.params.customerId || '0');
  ...
  router.post('/config', async (req: Request, res: Response) => {
    try {
      ...
      const existingConfig = db.getSsoConfig(customer.id);
   ...
   */
  router.get('/config', requireSsoAuth(db), (req: Request, res: Response) => {
    try {
      const customer = (req as any).customer;

      const ssoConfig = db.getSsoConfig(customer.id);
      if (!ssoConfig) {
        return res.status(404).json({
          success: false,
   ...
   */
  router.get('/sessions', requireSsoAuth(db), (req: Request, res: Response) => {
    try {
      const customer = (req as any).customer;

      const sessions = db.getActiveSessions(customer.id);
   ...
   */
  router.get('/stats', requireSsoAuth(db), (req: Request, res: Response) => {
    try {
      const customer = (req as any).customer;
      const days = parseInt(req.query.days as string) || 30;

      const stats = db.getSsoStats(customer.id, days);
