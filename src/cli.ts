...
async function checkAndInstallSnowCode(): Promise<boolean> {
  ...
  console.log(chalk.dim('Installing @groeimetai/snowcode@0.15.25...'));

  try {
    const projectDir = process.cwd();
    execSync('npm install @groeimetai/snowcode@0.15.25', {
      cwd: projectDir,
      stdio: 'inherit'
    });
    ...
    console.log(chalk.yellow('Please install it manually: ') + chalk.cyan('npm install @groeimetai/snowcode@0.15.25'));
  ...
  try {
    ...
    for (const [serverName, serverConfig] of Object.entries(finalConfig.mcpServers) as [string, any][]) {
      ...
      // Unknown format - copy as-is
        ...
        Object.assign(transformedConfig, serverConfig);
      }
