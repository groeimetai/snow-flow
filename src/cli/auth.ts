      ...
      try {
        ...
        try {
          ...
          if (enterpriseCreds && enterpriseCreds.type === 'enterprise') {
            ...
            // 🆕 ASK FOR ENTERPRISE INTEGRATION CREDENTIALS (Jira/Azure/Confluence)
            prompts.log.message('');
            prompts.log.step('Configure Enterprise Integrations (Jira, Azure DevOps, Confluence)');
            prompts.log.info('These credentials enable AI to work with your project management tools.');
            prompts.log.info('Skip any integration you don\'t use (press Enter to skip).');
            prompts.log.message('');

            try {
              const configureIntegrations = await prompts.confirm({
                message: 'Configure enterprise integrations now?',
                initialValue: true
              });
              ...
              if (configureIntegrations) {
                ...
                try {
                  ...
                  // .env doesn't exist yet, that's OK
                }

                // Parse existing .env to preserve other values
                ...
                // JIRA CONFIGURATION
                prompts.log.message('');
                prompts.log.info('━━━ Jira Configuration ━━━');
                ...
                if (configureJira) {
                  const jiraUrl = await prompts.text({
                    message: 'Jira Base URL (e.g., your-company.atlassian.net):',
                    placeholder: 'company.atlassian.net',
                    validate: (value) => {
                      if (!value) return 'Jira URL is required';
                      // Strip protocol if user included it
                      const cleaned = value.replace(/^https?:\/\//, '');
                      if (!cleaned.includes('.atlassian.net') && !cleaned.includes('jira.')) {
                  ...
                  // Clean and store
                  envVars['SNOW_JIRA_BASE_URL'] = String(jiraUrl).replace(/^https?:\/\//, '').replace(/\/$/, '');
                  envVars['SNOW_JIRA_EMAIL'] = String(jiraEmail);
                  envVars['SNOW_JIRA_API_TOKEN'] = String(jiraToken);

                  prompts.log.success('✓ Jira credentials configured');
                }

                // AZURE DEVOPS CONFIGURATION
                prompts.log.message('');
                prompts.log.info('━━━ Azure DevOps Configuration ━━━');
                ...
                if (configureAzure) {
                  ...
                  envVars['SNOW_AZURE_ORG'] = String(azureOrg);
                  envVars['SNOW_AZURE_PAT'] = String(azurePat);

                  prompts.log.success('✓ Azure DevOps credentials configured');
                }

                // CONFLUENCE CONFIGURATION
                prompts.log.message('');
                prompts.log.info('━━━ Confluence Configuration ━━━');
                ...
                if (configureConfluence) {
                  const confluenceUrl = await prompts.text({
                    message: 'Confluence Base URL (e.g., your-company.atlassian.net/wiki):',
                    placeholder: 'company.atlassian.net/wiki',
                    validate: (value) => {
                  ...
                  envVars['SNOW_CONFLUENCE_BASE_URL'] = String(confluenceUrl).replace(/^https?:\/\//, '').replace(/\/$/, '');
                  envVars['SNOW_CONFLUENCE_EMAIL'] = String(confluenceEmail);
                  envVars['SNOW_CONFLUENCE_API_TOKEN'] = String(confluenceToken);

                  prompts.log.success('✓ Confluence credentials configured');
                }
                ...
                prompts.log.message('');
                prompts.log.success('Enterprise integration credentials saved to .env');

                // Auto-sync credentials to portal
                prompts.log.message('');
                prompts.log.step('Syncing credentials to Enterprise Portal...');

                try {
                  // Read credentials we just wrote
                  const licenseKey = enterpriseCreds.licenseKey;
                  const enterpriseUrl = 'https://portal.snow-flow.dev';

                  // Get username/password from enterprise creds
                  const username = enterpriseCreds.username;
                  const password = enterpriseCreds.password;

                  if (!username || !password) {
                    throw new Error('Enterprise username/password not found');
                  }
                  ...
                  for (const cred of credentialsToSync) {
                    ...
                    if (response.ok && result.success) {
                      prompts.log.success(`✓ ${cred.service} credentials synced to portal`);
                    } else {
                      prompts.log.warn(`⚠️  ${cred.service} sync failed: ${result.error || 'Unknown error'}`);
                    }
                  }

                  prompts.log.success('Credentials synced to Enterprise Portal');
                } catch (syncError: any) {
                  prompts.log.warn(`⚠️  Auto-sync failed: ${syncError.message}`);
                  prompts.log.info('You can manually sync later with: snow-flow auth sync');
                }
              }
            } catch (integrationErr: any) {
              prompts.log.warn(`⚠️  Integration configuration skipped: ${integrationErr.message}`);
            }
          }
        } catch (err: any) {
