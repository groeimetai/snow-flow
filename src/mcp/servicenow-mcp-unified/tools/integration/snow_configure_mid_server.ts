  ...
  try {
    ...
    if (action === 'list') {
      ...
      var summary = {
        ...
        up: enrichedServers.filter(function(m: any) { return m.status === 'Up'; }).length,
        down: enrichedServers.filter(function(m: any) { return m.status === 'Down'; }).length,
        validated: enrichedServers.filter(function(m: any) { return m.validated; }).length,
        not_validated: enrichedServers.filter(function(m: any) { return !m.validated; }).length
      };
